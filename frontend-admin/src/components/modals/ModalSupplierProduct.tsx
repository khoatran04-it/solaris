    ...
    useEffect(() => {
        if (isOpen) {
            Promise.all([
                ...
                setSupplierOptions(suppliers.map(s => ({ value: s.id, label: `${s.code} - ${s.name}` })));
                    ...
                    <form id="supplierProductForm" onSubmit={handleSubmit} className="flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <FormInput
                                ...
                                disabled={isSubmitting}
                                onChange={e => handleFieldChange('lastImportPrice', parseFloat(e.target.value) || 0)}
                            />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <FormInput
                                ...
                                disabled={isSubmitting}
                                onChange={e => handleFieldChange('minimumOrderQuantity', parseFloat(e.target.value) || 1)}
                            />
                        ...
                        <div className="flex items-center gap-3 mt-1 bg-slate-50 p-4 rounded-xl border border-slate-100">
                            ...
                            <div className="flex flex-col">
                                <span className="text-[13px] font-bold text-slate-700">Đang Cung Ứng (Kích hoạt)</span>
                                <span className="text-[11px] text-slate-500">Cho phép chọn sản phẩm này khi tạo đơn mua hàng từ NCC.</span>
                            </div>
                        </div>
