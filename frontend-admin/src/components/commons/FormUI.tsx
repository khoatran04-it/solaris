...
export const PageContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="h-full flex flex-col p-6 bg-slate-50/30 overflow-y-auto font-sans">
    <div className="max-w-5xl w-full mx-auto flex flex-col gap-8 pb-12">{children}</div>
  </div>
);
...
export const FormHeader: React.FC<{
  ...
  onBack: () => void;
  icon?: any;
}> = ({ title, subtitle, onBack, icon: Icon }) => (
  <div className="flex justify-between items-end mb-2">
    ...
    <button
      type="button"
      onClick={onBack}
      className="group bg-white border border-slate-200 text-slate-600 rounded-xl px-5 py-2.5 text-sm font-bold flex items-center gap-2 hover:bg-slate-50 hover:text-slate-900 transition-all duration-300"
    >
      <ChevronLeft
        size={18}
        strokeWidth={2.5}
...
export const FormSelect: React.FC<FormSelectProps> = ({
  ...
  return (
    <div className="flex flex-col relative" ref={containerRef}>
      ...
      >
        <span
          className={`truncate ${selectedOption ? (disabled ? 'text-slate-500 font-medium' : 'text-slate-800 font-medium') : 'text-slate-400 italic'}`}
        >
          {selectedOption ? selectedOption.label : placeholder}
        </span>
        <ChevronDown
          size={16}
          className={`shrink-0 transition-transform duration-300 ${disabled ? 'text-slate-300' : 'text-slate-400'} ${isOpen && !disabled ? 'rotate-180 text-yellow-500' : ''}`}
        />
      </div>
