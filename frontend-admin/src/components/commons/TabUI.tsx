...
export const DetailPageContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="h-full flex flex-col p-6 bg-slate-50/30 overflow-y-auto font-sans">
    <div className="max-w-6xl w-full mx-auto flex flex-col gap-6 pb-12">{children}</div>
  </div>
);
...
export const DetailSection: React.FC<{
  ...
  dotColor?: string;
}> = ({ title, children, dotColor = 'bg-yellow-400' }) => (
...
export const InfoField: React.FC<{ label: string; value: React.ReactNode; className?: string }> = ({
  label,
  value,
  className = '',
}) => (
...
export const DetailProfileCard: React.FC<ProfileCardProps> = ({
  ...
  <div className="bg-slate-50/60 rounded-2xl p-6 border border-slate-100/70 flex flex-col items-center text-center sticky top-0">
    ...
    >
      <span
        className={`w-2 h-2 rounded-full ${statusActive ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}
      ></span>
...
export const TabEmptyPlaceholder: React.FC<{ title: string; desc: string; icon: LucideIcon }> = ({
  title,
  desc,
  icon: Icon,
}) => (
