    ...
    export const ListPageContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => (
        <div className="h-full flex flex-col p-6 bg-slate-50/30 overflow-y-auto font-sans">
            <div className="max-w-[1600px] w-full mx-auto flex flex-col gap-8 pb-12">
                {children}
            </div>
        </div>
    );
    ...
    export const ListHeader: React.FC<ListHeaderProps> = ({ title, subtitle, searchTerm, onSearchChange, onAdd, searchPlaceholder, icon: Icon }) => (
        <div className="flex justify-between items-end mb-2">
            ...
            <div className="flex gap-4">
                <div className="flex items-center bg-white/80 backdrop-blur-md border border-slate-200/80 rounded-xl px-4 py-2.5 w-80 focus-within:border-yellow-400 focus-within:ring-4 focus-within:ring-yellow-400/20 transition-all duration-300 shadow-sm">
                    ...
                    <input 
                        type="text" placeholder={searchPlaceholder || "Tìm kiếm..."} value={searchTerm}
                ...
                <button onClick={onAdd} className="group bg-yellow-400 hover:bg-yellow-500 text-slate-900 border-none rounded-xl px-6 py-2.5 text-[15px] flex items-center gap-2 cursor-pointer font-bold transition-all duration-300 shadow-[0_4px_14px_0_rgba(250,204,21,0.39)] hover:shadow-[0_6px_20px_rgba(250,204,21,0.23)] hover:-translate-y-0.5">
                    <Plus size={18} strokeWidth={3} className="transition-transform group-hover:rotate-90" /> THÊM
                </button>
            </div>
        </div>
    );
        ...
        <tr>
            <td colSpan={colSpan} className="py-24 text-center bg-slate-50/30">
                <div className="flex flex-col items-center justify-center text-slate-400">
                    ...
                    <span className="text-sm font-medium text-slate-400">{message || 'Thử thay đổi từ khóa tìm kiếm hoặc điều kiện lọc.'}</span>
                </div>
            </td>
        </tr>
    );
    ...
    export const ListPagination: React.FC<PaginationProps> = ({ currentPage, totalPages, onPageChange, totalItems, isLoading }) => (
