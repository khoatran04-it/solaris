...
export const CustomDateFilter: React.FC<CustomDateFilterProps> = ({ title, selectedDate, onApply }) => {
            ...
            {isOpen && (
                <div className="absolute top-full right-0 mt-3 w-72 bg-white border border-slate-100 rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] z-50 p-5 font-normal animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="grid grid-cols-7 gap-1 mb-5">
                        ...
                        {days.map(day => {
                            const isSelected = tempDate?.getDate() === day && tempDate?.getMonth() === currentMonth.getMonth() && tempDate?.getFullYear() === currentMonth.getFullYear();
                            const isToday = new Date().getDate() === day && new Date().getMonth() === currentMonth.getMonth() && new Date().getFullYear() === currentMonth.getFullYear();
                            
                            return (
                                <div 
                                    key={day}
                                    onClick={() => setTempDate(new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day))}
                                    className={`
                    ...
                    <div className="flex border-t border-slate-100 pt-4 gap-3">
                        <button 
                            onClick={() => { setTempDate(null); onApply(null); setIsOpen(false); }}
                        ...
                        <button 
                            onClick={() => { onApply(tempDate); setIsOpen(false); }}
