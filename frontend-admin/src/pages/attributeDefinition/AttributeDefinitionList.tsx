...
const AttributeDefinitionList: React.FC = () => {
  ...
  const getDataTypeBadge = (type: string) => {
    switch (type.toUpperCase()) {
      case 'NUMBER':
        return (
          <span className="inline-flex px-2 py-1 rounded bg-blue-50 text-blue-600 border border-blue-100 font-semibold text-[11px]">
            Số (Number)
          </span>
        );
      case 'OPTIONS':
        return (
          <span className="inline-flex px-2 py-1 rounded bg-fuchsia-50 text-fuchsia-600 border border-fuchsia-100 font-semibold text-[11px]">
            Chọn sẵn (Options)
          </span>
        );
      case 'TEXT':
      default:
        return (
          <span className="inline-flex px-2 py-1 rounded bg-slate-100 text-slate-600 border border-slate-200 font-semibold text-[11px]">
            Văn bản (Text)
          </span>
        );
    }
              ...
              <tr className="bg-slate-50/70 border-b border-slate-100">
                ...
                <th className="w-[15%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                  <CustomFilter
                    ...
                    selectedValues={dataTypeFilter}
                    onApply={setDataTypeFilter}
                  />
                </th>
                ...
                data.map((item) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-6">
                      ...
                      <div className="text-[11px] text-slate-400 mt-0.5">ID: {item.id}</div>
                    </td>

                    {/* KIỂU DỮ LIỆU */}
                    <td className="py-3 px-2">{getDataTypeBadge(item.dataType)}</td>
