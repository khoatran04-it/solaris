            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6 p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Tham số Báo cáo:</span>
                    </div>
                    ...
                    {shiftData && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Box size={48} /></div>
                                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider relative z-10">Tồn Đầu Kỳ</div>
                                <div className="text-3xl font-black text-slate-800 mt-2 relative z-10">{shiftData.totalOpeningItems.toLocaleString('vi-VN')}</div>
                            </div>
                            <div className="p-5 bg-emerald-500 border border-emerald-600 rounded-2xl shadow-sm shadow-emerald-200 relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-4 opacity-20 group-hover:scale-110 transition-transform"><ArrowDownRight size={48} className="text-emerald-900" /></div>
                                <div className="text-xs font-bold text-emerald-100 uppercase tracking-wider relative z-10">Tổng Phát Sinh Nhập (+)</div>
                                <div className="text-3xl font-black text-white mt-2 relative z-10">+{shiftData.totalInflowItems.toLocaleString('vi-VN')}</div>
                            </div>
                            <div className="p-5 bg-rose-500 border border-rose-600 rounded-2xl shadow-sm shadow-rose-200 relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-4 opacity-20 group-hover:scale-110 transition-transform"><ArrowUpRight size={48} className="text-rose-900" /></div>
                                <div className="text-xs font-bold text-rose-100 uppercase tracking-wider relative z-10">Tổng Phát Sinh Xuất (-)</div>
                                <div className="text-3xl font-black text-white mt-2 relative z-10">-{shiftData.totalOutflowItems.toLocaleString('vi-VN')}</div>
                            </div>
                            <div className="p-5 bg-indigo-600 border border-indigo-700 rounded-2xl shadow-sm shadow-indigo-200 relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-4 opacity-20 group-hover:scale-110 transition-transform"><Scale size={48} className="text-indigo-900" /></div>
                                <div className="text-xs font-bold text-indigo-200 uppercase tracking-wider relative z-10">Tồn Cuối Kỳ Lý Thuyết</div>
                                <div className="text-3xl font-black text-white mt-2 relative z-10">{shiftData.totalClosingItems.toLocaleString('vi-VN')}</div>
                            </div>
                        </div>
                    )}
                            ...
                            <table className="w-full text-left whitespace-nowrap text-sm min-w-375">
                                <thead className="bg-slate-50 text-slate-600 font-bold uppercase tracking-wider border-b border-slate-200 text-xs">
                                    <tr>
                                        ...
                                        <th className="px-4 py-4 text-center bg-slate-100/80 min-w-25 border-l border-slate-200">Tồn Đầu</th>
                                        
                                        <th className="px-4 py-4 text-center text-emerald-800 bg-emerald-50/60 min-w-25 border-l border-emerald-100">Nhập Mua</th>
                                        <th className="px-4 py-4 text-center text-emerald-800 bg-emerald-50/60 min-w-25">Chuyển Đến</th>
                                        <th className="px-4 py-4 text-center text-emerald-800 bg-emerald-50/60 min-w-25">Khách Trả</th>
                                        
                                        <th className="px-4 py-4 text-center text-rose-800 bg-rose-50/60 min-w-25 border-l border-rose-100">Xuất Bán</th>
                                        <th className="px-4 py-4 text-center text-rose-800 bg-rose-50/60 min-w-25">Chuyển Đi</th>
                                        
                                        <th className="px-4 py-4 text-center text-amber-800 bg-amber-50/60 min-w-27.5 border-l border-amber-100">Điều Chỉnh (±)</th>
                                        
                                        <th className="px-4 py-4 text-center font-black text-indigo-900 bg-indigo-100/60 min-w-30 border-l border-indigo-200">Tồn Cuối Kỳ</th>
                                        
                                        <th className="px-4 py-4 text-center text-emerald-700 min-w-25 border-l border-slate-200">Khả Dụng</th>
                                    ...
                                    {shiftLoading ? (
                                        ...
                                        shiftData.items.map((row, idx) => (
                                            <tr key={row.variantId} className="hover:bg-slate-50/70 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-slate-700 bg-slate-50/40 border-l border-slate-100">{row.openingStock}</td>
                                                
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-emerald-600 bg-emerald-50/20 border-l border-emerald-50">{row.totalReceipt > 0 ? `+${row.totalReceipt}` : '-'}</td>
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-emerald-600 bg-emerald-50/20">{row.totalTransferIn > 0 ? `+${row.totalTransferIn}` : '-'}</td>
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-emerald-600 bg-emerald-50/20">{row.totalReturn > 0 ? `+${row.totalReturn}` : '-'}</td>
                                                
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-rose-600 bg-rose-50/20 border-l border-rose-50">{row.totalIssue > 0 ? `-${row.totalIssue}` : '-'}</td>
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-rose-600 bg-rose-50/20">{row.totalTransferOut > 0 ? `-${row.totalTransferOut}` : '-'}</td>
                                                ...
                                                <td className="px-4 py-3 text-center font-black text-[15px] text-indigo-900 bg-indigo-50/50 border-l border-indigo-100">{row.closingStock}</td>
                                                
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-emerald-700 border-l border-slate-100">{row.currentAvailable}</td>
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-indigo-700">{row.currentReserved}</td>
                                                <td className="px-4 py-3 text-center font-bold text-[13px] text-rose-700">{row.currentDamaged}</td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
                            ...
                            <table className="w-full text-left whitespace-nowrap text-sm min-w-300">
                                <thead className="bg-slate-50 text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="px-4 py-4 text-center min-w-30 bg-indigo-50/50 text-indigo-900">Biến Động</th>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {ledgerLoading ? (
                                        ...
                                        <TableEmpty colSpan={10} message="Chưa có giao dịch sổ cái nào trong khoảng thời gian này." />
                                    ) : (
                                        ledgerEntries.map((entry, idx) => {
                                            const isPositive = entry.quantity > 0;
                                            return (
                                                <tr key={entry.transactionCode || idx} className="hover:bg-slate-50/80 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3 text-center bg-indigo-50/20 border-l border-indigo-50">
                                                        <span className={`text-[15px] font-black ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                    ...
                                                    <td className="px-4 py-3 text-[13px] italic text-slate-500 max-w-xs truncate" title={entry.note}>
                                                        {entry.note || '---'}
                                                    </td>
                                                </tr>
                                            );
