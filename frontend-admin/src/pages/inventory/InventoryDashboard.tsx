...
import { 
    Package, History, AlertTriangle, CheckCircle, 
    Warehouse as WarehouseIcon, Search, Filter, ShieldAlert 
} from 'lucide-react';
...
import { 
    ListPageContainer, ListCard, TableLoading, 
    TableEmpty, ListPagination 
...
const InventoryDashboard: React.FC = () => {
    ...
    useEffect(() => {
        warehouseApi.getAllList()
            ...
            .catch(() => showToast('error', 'Không tải được danh sách Kho hàng. Kiểm tra lại phân quyền!'));
    }, []);
    ...
    const renderExpiryBadge = (days: number) => {
        if (days < 0) return <span className="bg-rose-100 text-rose-700 px-2 py-0.5 rounded text-[10px] font-bold border border-rose-200">ĐÃ HẾT HẠN</span>;
        if (days <= 3) return <span className="bg-red-500 text-white px-2 py-0.5 rounded text-[10px] font-bold shadow-sm animate-pulse">BÁO ĐỘNG ({days} ngày)</span>;
        if (days <= 7) return <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded text-[10px] font-bold border border-amber-200">CẬN DATE ({days} ngày)</span>;
        return <span className="bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded text-[10px] font-bold border border-emerald-100">AN TOÀN</span>;
    };
        ...
        <ListPageContainer>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Giám sát số lượng vật lý và chất lượng lô hàng theo thời gian thực.</p>
                </div>
            ...
            <div className={activeTab === 'inventory' ? 'block animate-in fade-in duration-300' : 'hidden'}>
                <ListCard>
                    ...
                    <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row gap-4 items-center justify-between bg-slate-50/50">
                        ...
                        <div className="flex gap-3 w-full md:w-auto">
                            ...
                            >
                                <AlertTriangle size={16} className={isExpiringSoon ? 'text-amber-600' : 'text-slate-400'} />
                                Hàng Sắp Hết Hạn
                            </button>
                            <button 
                        ...
                        <table className="w-full text-left border-collapse min-w-250">
                            <thead>
                                <tr className="bg-slate-100/50 border-b border-slate-200">
                                    <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase">Thông Tin Mặt Hàng</th>
                                    <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase">Chi Tiết Lô & HSD</th>
                                    <th className="py-4 px-4 text-xs font-bold text-emerald-600 uppercase text-right bg-emerald-50/50">Khả Dụng</th>
                                    <th className="py-4 px-4 text-xs font-bold text-amber-600 uppercase text-right bg-amber-50/50">Giữ Chỗ</th>
                                    <th className="py-4 px-4 text-xs font-bold text-rose-600 uppercase text-right bg-rose-50/50">Lỗi/QC</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {isLoading ? (
                                        ...
                                        <tr key={item.id} className="hover:bg-indigo-50/30 transition-colors group">
                                            ...
                                            <td className="py-4 px-6">
                                                <div className="flex flex-col">
                                                    <span className="font-bold text-[14px] text-slate-800">{item.variantName}</span>
                                            ...
                                            <td className="py-4 px-4">
                                                <div className="flex flex-col gap-1.5">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[12px] font-bold text-slate-700">{item.batchCode}</span>
                                                        {renderExpiryBadge(item.daysToExpiry)}
                                                    </div>
                                                    <div className="text-[11px] text-slate-500">
                                                        NSX: {formatDate(item.manufactureDate)} &nbsp;|&nbsp; HSD: <strong className="text-slate-700">{formatDate(item.expiryDate)}</strong>
                                                    </div>
                                                    {item.supplierName && (
                                                        <div className="text-[11px] text-slate-400 truncate w-48" title={item.supplierName}>
                                            ...
                                            <td className="py-4 px-4 text-right bg-emerald-50/10 group-hover:bg-emerald-50/40">
                                                <span className={`text-[16px] font-black ${item.quantityAvailable > 0 ? 'text-emerald-600' : 'text-slate-300'}`}>
                                            ...
                                            <td className="py-4 px-4 text-right bg-amber-50/10 group-hover:bg-amber-50/40">
                                                <span className={`text-[15px] font-bold ${item.quantityReserved > 0 ? 'text-amber-600' : 'text-slate-300'}`}>
                                            ...
                                            <td className="py-4 px-4 text-right bg-rose-50/10 group-hover:bg-rose-50/40">
                                                <div className="flex flex-col items-end">
                                                    <span className={`text-[14px] font-bold ${item.quantityDamaged > 0 ? 'text-rose-600' : 'text-slate-300'}`} title="Hàng hỏng">
                                                        {item.quantityDamaged.toLocaleString('vi-VN')}
                                                    </span>
                                                    {item.quantityQC > 0 && (
                                                        <span className="text-[11px] text-orange-500 font-medium mt-0.5" title="Hàng chờ kiểm định (QC)">
                                                            + {item.quantityQC} QC
                                                        </span>
                                                    )}
                                                </div>
                                    ...
                                    <TableEmpty colSpan={5} message={selectedWarehouseId === 0 ? "Vui lòng chọn Kho hàng để xem dữ liệu." : "Không tìm thấy dữ liệu tồn kho phù hợp."} />
                                )}
                            </tbody>
                        </table>
                    </div>
            ...
            <div className={activeTab === 'history' ? 'block animate-in fade-in duration-300' : 'hidden'}>
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-16 flex flex-col items-center justify-center text-center">
                    ...
                    <h2 className="text-2xl font-black text-slate-800 mb-2">Tính năng đang được phát triển</h2>
                    <p className="text-slate-500 max-w-lg mx-auto leading-relaxed">
                        Màn hình sao kê <strong>Lịch sử Nhập / Xuất kho</strong> (kèm chức năng bấm xem chi tiết lô hàng nhập trong ngày) sẽ được tích hợp ngay khi hoàn thành <strong>Phase 3 (Module Nhập Kho)</strong>.
                    </p>
