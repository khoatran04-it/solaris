...
import { UoMCategory } from '../../types/uomCategory';
...
const UoMCategoryList: React.FC = () => {
    const navigate = useNavigate();

    // --- STATE QUẢN LÝ DỮ LIỆU & PHÂN TRANG ---
    const [data, setData] = useState<UoMCategory[]>([]);
    ...
    const [deletingRecord, setDeletingRecord] = useState<UoMCategory | null>(null);
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const response = await uomCategoryApi.getAll({
    ...
    const confirmDelete = async () => {
        ...
        try {
            await uomCategoryApi.delete(deletingRecord.id);
            setIsModalOpen(false);
            fetchData();
            showToast('success', `Xóa thành công nhóm "${deletingRecord.name}"`);
        } catch (error: any) {
            showToast('error', error?.response?.data?.message || 'Lỗi khi thực hiện xóa dữ liệu. Đảm bảo nhóm này không chứa đơn vị nào.');
        }
    };

    const handleToggleActive = async (id: number, currentStatus: boolean) => {
        try {
            await uomCategoryApi.toggleActive(id);
            fetchData();
            showToast('success', `Đã ${currentStatus ? 'tạm khóa' : 'kích hoạt'} nhóm đơn vị tính`);
        } catch (error) {
    ...
    return (    
        <ListPageContainer>
            ...
            <ListHeader 
                ...
                subtitle="Quản lý các nhóm quy đổi tiêu chuẩn (VD: Khối lượng, Chiều dài...)"
                searchTerm={searchTerm}
                onSearchChange={setSearchTerm}
                onAdd={() => navigate('/uom-categories/create')}
                icon={Layers}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                <th className="w-[20%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã Nhóm</th>
                                <th className="w-[30%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Tên Nhóm</th>
                                ...
                                data.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                        <td className="py-4 px-6">
                                            <span className="inline-flex px-2.5 py-1 rounded-md bg-indigo-50 text-indigo-700 text-sm font-bold border border-indigo-200/50 uppercase tracking-wider">
                                                {item.code}
                                            </span>
                                        </td>
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex justify-center gap-1.5 opacity-40 group-hover:opacity-100 transition-all duration-300">
                                                <button 
                                                    onClick={() => navigate(`/uom-categories/edit/${item.id}`)}
...
export default UoMCategoryList;