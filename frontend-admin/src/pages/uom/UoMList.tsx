...
const UoMList: React.FC = () => {
    ...
    }, [currentPage, debouncedSearch, categoryFilter, statusFilter, createdAtFilter, updatedAtFilter]);
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                <th className="w-[15%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Đơn vị tính</th>
                                <th className="w-[20%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Từ đồng nghĩa (AI)</th>
                                
                                <th className="w-[15%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <CustomFilter title="THUỘC NHÓM" options={categoryOptions} selectedValues={categoryFilter} onApply={setCategoryFilter} />
                                </th>
                                    ...
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                        <td className="py-4 px-6">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[11px] font-bold text-indigo-600 mt-0.5">{item.code}</span>
                                            </div>
                                        </td>
                                            ...
                                            {item.synonyms ? (
                                                <div className="flex flex-wrap gap-1.5">
                                                    {item.synonyms.split(',').map((syn, idx) => (
                                                        <span key={idx} className="inline-flex px-1.5 py-0.5 rounded text-[11px] font-medium bg-slate-100 text-slate-500 border border-slate-200">
                                                            {syn.trim()}
                                                        </span>
                                                    ))}
                                                </div>
