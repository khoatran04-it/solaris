    ...
    useEffect(() => {
        const loadInitData = async () => {
            try {
                ...
                setCustomers(custList.map((c: any) => ({ value: c.id, label: `${c.code} - ${c.name} (${c.phoneNumber})` })));
    ...
    const handlePreviewRouting = async () => {
        ...
        try {
            ...
            const payload: OrderCreatePayload = {
                ...
                customerAddressId: formData.customerAddressId ? Number(formData.customerAddressId) : undefined,
            ...
            if (result.optimalWarehouseId) {
                ...
                showToast('success', `Đã chọn kho tối ưu: ${result.warehouseName} (Cách ${result.distanceKm} km)`);
            }
        } catch (err: any) {
                            ...
                            <table className="w-full text-left text-sm whitespace-nowrap">
                                <thead className="bg-slate-50 text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="px-3 py-3 w-28 text-center bg-indigo-50/40">Số lượng <span className="text-red-500">*</span></th>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {details.map((row, idx) => {
                                        const lineTotal = (Number(row.quantity || 0) * Number(row.unitPrice || 0)) - Number(row.discountAmount || 0);
                                        return (
                                            <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="p-2">
                                                    <FormSelect
                                                        ...
                                                        onSelect={(val) => handleDetailChange(row.id, 'variantId', val ? Number(val) : '')}
                                                    />
                                                </td>
                                                ...
                                                <td className="p-2">
                                                    <FormInput
                                                        ...
                                                        onChange={(e) => handleDetailChange(row.id, 'discountAmount', parseFloat(e.target.value) || 0)}
                                                    />
                                                </td>
                            ...
                            <div className="p-6 bg-indigo-50/40 border border-indigo-100 rounded-2xl flex flex-col justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-600 mb-4 leading-relaxed">
                                        Hệ thống sẽ tính khoảng cách Haversine từ địa chỉ khách hàng tới các kho và kiểm tra tồn kho khả dụng để tự động chọn kho tối ưu nhất.
                                    </p>
                                    ...
                                    {routingPreview && (
                                        <div className="p-4 bg-white rounded-xl border border-indigo-200 text-xs flex flex-col gap-2.5 shadow-sm">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="font-black text-indigo-700 text-sm">{routingPreview.warehouseName} ({routingPreview.distanceKm} km)</span>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                ...
                                                {routingPreview.isFullyStocked ? (
                                                    ...
                                                    <span className="text-amber-700 font-bold flex items-center gap-1 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200">
                                                        <AlertTriangle size={13} /> Thiếu {routingPreview.missingItems.length} mặt hàng
                                                    </span>
                                                )}
                                            </div>
                                            {routingPreview.suggestedSourceWarehouseName && (
                                                <div className="pt-2 border-t border-slate-100 text-indigo-900 font-medium">
                                                    💡 Gợi ý điều phối: Có thể lập lệnh chuyển kho từ <strong>{routingPreview.suggestedSourceWarehouseName}</strong> về.
                                                </div>
                                            )}
                                        </div>
                                    )}
                            ...
                            <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col justify-between">
                                ...
                                <div className="pt-4 border-t border-slate-200 flex flex-col gap-2 mt-4">
                                    ...
                                    <div className="flex justify-between text-sm text-slate-600">
                                        ...
                                        <span className="font-bold text-slate-800">{Number(formData.shippingFee || 0).toLocaleString('vi-VN')} đ</span>
                                    </div>
