...
const RoleForm: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Lấy chi tiết Edit
        if (isEditMode && id) {
            roleApi.getById(Number(id)).then(res => {
                setFormData({
                    code: res.code || '',
                    name: res.name || '',
                    ...
                    <div className={activeTab === 'info' ? 'block animate-in fade-in slide-in-from-bottom-4 duration-300' : 'hidden'}>
                        <FormSection title="Định Danh Vai Trò">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <FormInput 
                                    label="Mã Vai Trò (Định danh)" required placeholder="VD: SALE_MANAGER"
                                    value={formData.code || ''} error={errors.code} disabled={isEditMode || loading}
                                    onChange={e => handleFieldChange('code', e.target.value)} 
                                />
                                <FormInput 
                                    label="Tên Vai Trò" required placeholder="VD: Trưởng phòng Kinh Doanh"
                                ...
                                <div className="md:col-span-2">
                                    <FormTextarea 
                                        label="Mô tả Vai trò" placeholder="Mô tả chức năng nhiệm vụ của vai trò này..." 
                    ...
                    <div className={activeTab === 'permissions' ? 'block animate-in fade-in slide-in-from-bottom-4 duration-300' : 'hidden'}>
                        <FormSection title="Phân Quyền Hệ Thống">
                            <p className="text-sm text-slate-500 mb-6 -mt-2">
                                Tích chọn các quyền mà vai trò này được phép thao tác. Các User thuộc vai trò này sẽ tự động thừa hưởng toàn bộ quyền được cấp dưới đây.
                            </p>
