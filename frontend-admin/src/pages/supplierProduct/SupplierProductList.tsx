...
const SupplierProductList: React.FC = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast(
        'success',
        `Đã xóa sản phẩm khỏi danh mục của NCC "${deletingRecord.supplierName}"`
      );
    } catch (error: any) {
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                      ...
                      <td className="py-3 px-6">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col min-w-0">
                            <span
                              className="font-bold text-slate-800 text-[13px] truncate"
                ...
                <TableEmpty
                  colSpan={7}
                  message="Chưa có cấu hình bảng giá nào hoặc không khớp với bộ lọc."
                />
              )}
            </tbody>
          </table>
