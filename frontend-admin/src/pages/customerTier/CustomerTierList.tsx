...
const CustomerTierList: React.FC = () => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            fetchData();
            showToast('success', `Xóa thành công phân bậc "${deletingRecord.name}"`);
        } catch (error) {
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                <th className="w-[10%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã</th>
                                <th className="w-[20%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Tên Phân Bậc</th>
                                <th className="w-[12%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Chiết Khấu</th>
                                <th className="w-[18%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Chi tiêu tối thiểu</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {isLoading ? (
                                ...
                                data.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                        ...
                                        <td className="py-4 px-6 text-right font-medium">
                                            {item.minSpending.toLocaleString('vi-VN')} đ
                                        </td>
                                ...
                                <TableEmpty colSpan={7} message="Không tìm thấy bậc khách hàng phù hợp." />
                            )}
                        </tbody>
                    </table>
                </div>
