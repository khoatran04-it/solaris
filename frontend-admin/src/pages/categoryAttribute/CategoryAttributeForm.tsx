    ...
    try {
      const cleanPayload: CategoryAttributePayload = {
        ...
        attributeDefinitionId:
          formData.attributeDefinitionId === 0 ? undefined : formData.attributeDefinitionId,
  ...
  return (
    <PageContainer>
      ...
      <FormHeader
        ...
        subtitle={
          isEditMode
            ? 'Thay đổi yêu cầu nhập liệu của thuộc tính'
            : 'Quy định các thuộc tính cần có cho một danh mục (VD: Danh mục Thịt heo -> Cần thuộc tính Sơ chế)'
        }
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
          <FormSection title="Thiết Lập Cấu Hình">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {/* Cột 2: Thuộc Tính */}
              <FormSelect
                label="Thuộc tính (Attribute)"
                required
                showSearch
                placeholder="-- Chọn thuộc tính --"
                value={formData.attributeDefinitionId || 0}
              ...
              <FormSelect
                label="Yêu cầu khi tạo biến thể"
                required
                value={formData.isRequired ? 1 : 0}
            ...
            {isEditMode && (
              <div className="mt-6 px-4 py-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-700 m-0">
                  <strong className="font-bold">Lưu ý:</strong> Trong chế độ chỉnh sửa, bạn chỉ có
                  thể thay đổi trạng thái <strong>Bắt buộc / Tùy chọn</strong>. Nếu muốn đổi sang
                  danh mục hoặc thuộc tính khác, vui lòng xóa cấu hình này và tạo mới.
                </p>
              </div>
            )}
          </FormSection>
