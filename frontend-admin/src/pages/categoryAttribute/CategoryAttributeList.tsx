...
const CategoryAttributeList: React.FC = () => {
    ...
    return (    
        <ListPageContainer>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                ...
                                <th className="w-[30%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">
                                    <CustomFilter title="DANH MỤC SẢN PHẨM" options={categoryOptions} selectedValues={categoryFilter} onApply={setCategoryFilter} />
                                </th>
                                
                                {/* Cột 2: Thuộc tính gắn kèm (Có Filter) */}
                                <th className="w-[30%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <CustomFilter title="THUỘC TÍNH (ATTRIBUTE)" options={attributeOptions} selectedValues={attributeFilter} onApply={setAttributeFilter} />
                                </th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {isLoading ? (
                                    ...
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                        ...
                                        <td className="py-3 px-6">
                                            <div className="font-extrabold text-slate-800 text-[14px]">
                                                {item.categoryName || <span className="text-slate-400 italic">Lỗi: Danh mục đã bị xóa</span>}
                                            </div>
                                        </td>
                                        ...
                                        <td className="py-3 px-2">
                                            <div className="inline-flex items-center px-2.5 py-1 rounded bg-indigo-50 text-indigo-700 border border-indigo-100 font-semibold text-[12px]">
                                                {item.attributeDefinitionName || <span className="italic text-slate-400">---</span>}
                                            </div>
                                        </td>
                                        ...
                                        <td className="py-3 px-2 text-center">
                                            <div className="flex justify-center">
                                                {item.isRequired ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-rose-50 text-rose-600 border border-rose-200/50 font-bold text-[11px]" title="Bắt buộc nhập khi tạo Biến thể">
                                                        <ShieldAlert size={14} />
                                                        Bắt buộc
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 border border-slate-200 font-medium text-[11px]" title="Không bắt buộc nhập khi tạo Biến thể">
                                                        <ShieldCheck size={14} />
                                                        Tùy chọn
                                                    </span>
                                                )}
                                ...
                                <TableEmpty colSpan={4} message="Chưa có cấu hình nào khớp với tìm kiếm. Hãy thêm mới!" />
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            <ConfirmDeleteModal 
                isOpen={isModalOpen} 
                itemName={deletingRecord ? `thuộc tính ${deletingRecord.attributeDefinitionName} khỏi ${deletingRecord.categoryName}` : ''} 
