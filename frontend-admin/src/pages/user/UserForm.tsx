...
import { UserPlus, Save, Plus, Info, Shield, KeyRound, ChevronRight, CheckSquare, Warehouse as WarehouseIcon } from 'lucide-react';
...
const UserForm: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Lấy chi tiết Edit
        if (isEditMode && id) {
            userApi.getById(Number(id)).then(res => {
                setFormData({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!validateForm()) return showToast('warning', 'Vui lòng kiểm tra lại các thông tin bắt buộc!');
        ...
        <PageContainer>
            ...
            <FormCard>
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-8">
                            ...
                            <FormSection title="Tài Khoản Đăng Nhập">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-slate-50/50 border border-slate-100 rounded-xl">
                                    <FormInput 
                                        label="Tên đăng nhập (Username)" required placeholder="VD: nguyenvan_a"
                                    ...
                                    {!isEditMode && (
                                        <FormInput 
                                            label="Mật khẩu khởi tạo" required type="text" placeholder="Nhập mật khẩu..."
                                            value={formData.password || ''} error={errors.password} disabled={loading}
                            ...
                            <FormSection title="Hồ Sơ Nhân Sự">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <FormInput 
                                        label="Họ và Tên" required placeholder="VD: Nguyễn Văn A"
                                        value={formData.fullName} error={errors.fullName} disabled={loading}
                                        onChange={e => handleFieldChange('fullName', e.target.value)} 
                                    />
                                    <FormInput 
                                        label="Số CCCD / CMND" required placeholder="Nhập 12 số CCCD..."
                                        value={formData.citizenId} error={errors.citizenId} disabled={loading}
                                        onChange={e => handleFieldChange('citizenId', e.target.value)} 
                                    />
                                    <FormInput 
                                        label="Email liên hệ" required placeholder="Email công ty hoặc cá nhân..."
                    ...
                    {/* ================= TAB 2: VAI TRÒ & KHO ================= */}
                    <div className={activeTab === 'roles' ? 'block animate-in fade-in slide-in-from-bottom-4 duration-300' : 'hidden'}>
                        <div className="flex flex-col gap-10">
                            
                            {/* PHẦN 1: GÁN VAI TRÒ */}
                            <FormSection title="Cấp Phát Vai Trò (Role)">
                                <p className="text-sm text-slate-500 mb-6 -mt-2">
                                    Chọn một hoặc nhiều vai trò. Nhân viên sẽ thừa hưởng toàn bộ quyền của các vai trò được chọn.
                                </p>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-start justify-between">
                                                    <span className={`font-bold text-[14px] ${isChecked ? 'text-indigo-800' : 'text-slate-700'}`}>
                                                        {role.name}
                                                    </span>
                                                    <div className={`shrink-0 transition-transform ${isChecked ? 'scale-110 text-indigo-600' : 'text-slate-300'}`}>
                                                        {isChecked ? <CheckSquare size={20} /> : <div className="w-5 h-5 border-2 border-slate-300 rounded-md" />}
                                                    </div>
                                                </div>
                            ...
                            <FormSection title="Phân Quyền Truy Cập Kho (Data-level Security)">
                                <p className="text-sm text-slate-500 mb-6 -mt-2">
                                    Nhân viên này được phép xem và thao tác dữ liệu (Phiếu nhập, xuất...) ở những kho nào?
                                </p>
                                <div className="flex flex-wrap gap-3">
                                    {allWarehouses.length === 0 ? (
                                        <span className="text-sm italic text-slate-400">Chưa có kho hàng nào trong hệ thống.</span>
                                    ) : (
                                        allWarehouses.map(warehouse => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <WarehouseIcon size={18} className={isChecked ? 'text-emerald-600' : 'text-slate-400'} />
                                                    <span className="text-[13px] font-bold">{warehouse.name}</span>
                                                    <div className={`ml-2 w-4 h-4 rounded border flex items-center justify-center transition-colors
                    ...
                    <div className={activeTab === 'permissions' ? 'block animate-in fade-in slide-in-from-bottom-4 duration-300' : 'hidden'}>
                        <FormSection title="Tặng Thêm Quyền Ngoại Lệ">
                            <div className="p-4 mb-6 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
                                ...
                                <p className="text-sm text-amber-800 leading-relaxed">
                                    <strong>Tính năng nâng cao:</strong> Bạn chỉ nên cấp quyền tại đây nếu muốn tặng thêm 1 vài quyền lặt vặt cho nhân viên này <strong>mà không cần phải tạo ra một Vai trò (Role) mới</strong>. <br/>
                    ...
                    <div className="flex justify-between items-center pt-6 border-t border-slate-100 mt-2">
                        ...
                        {activeTab !== 'permissions' ? (
                            ...
                            >
                                {activeTab === 'info' ? 'Cấp quyền & Kho' : 'Cấu hình ngoại lệ'} <ChevronRight size={18} />
                            </button>
                        ) : (
