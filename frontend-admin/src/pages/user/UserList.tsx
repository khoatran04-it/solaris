...
const UserList: React.FC = () => {
  ...
  const renderRoles = (roleIds: number[]) => {
    if (!roleIds || roleIds.length === 0)
      return <span className="text-slate-400 italic">Chưa cấp quyền</span>;
    ...
    return (
      <div className="flex flex-wrap gap-1.5">
        {names.slice(0, 2).map((name, idx) => (
          <span
            key={idx}
            className="px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-200/60 rounded text-[11px] font-bold"
          >
            {name}
          </span>
        ))}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-100">
                <th className="w-[30%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">
                  Nhân Viên
                </th>
                ...
                <th className="w-[13%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">
                  Đăng Nhập Cuối
                </th>
