...
const InventoryTransferList: React.FC = () => {
    ...
    }, [debouncedSearch, currentPage, fromWhFilter, toWhFilter, statusFilter, fromDateFilter, toDateFilter]);
                    ...
                    <table className="w-full text-left border-collapse min-w-250">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                <th className="w-[12%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã Phiếu</th>
                                
                                <th className="w-[16%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <CustomFilter title="KHO XUẤT" options={warehouseOptions} selectedValues={fromWhFilter} onApply={setFromWhFilter} />
                                </th>
                                
                                <th className="w-[16%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <CustomFilter title="KHO NHẬN" options={warehouseOptions} selectedValues={toWhFilter} onApply={setToWhFilter} />
                                </th>
                                ...
                                <th className="w-[10%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider hidden lg:table-cell">Người Lập</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                transfers.map((transfer) => (
                                    <tr key={transfer.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                        {/* CELL 1: MÃ PHIẾU */}
                                        ...
                                        <td className="py-3 px-2">
                                            <div className="text-[13px] font-bold text-slate-800">{transfer.fromWarehouseName}</div>
                                        </td>
                                        
                                        {/* CELL 3: KHO NHẬN */}
                                        <td className="py-3 px-2">
                                            <div className="text-[13px] font-bold text-slate-800">{transfer.toWarehouseName}</div>
                                        </td>
                                        ...
                                        <td className="py-3 px-2 text-center">
                                            <div className="flex justify-center">
                                                <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold border shadow-sm ${InventoryTransferStatusColors[transfer.status]}`}>
