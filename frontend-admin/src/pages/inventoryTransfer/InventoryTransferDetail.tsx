...
const InventoryTransferDetail: React.FC = () => {
    ...
    if (!transfer) return <div className="p-12 text-center text-rose-500 font-bold">Không tìm thấy chứng từ chuyển kho!</div>;
    ...
    return (
        <DetailPageContainer>
            ...
            <DetailHeader
                ...
                subtitle={<>Mã chứng từ: <span className="font-bold text-slate-800">{transfer.transferCode}</span></>}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6 p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border shadow-sm ${InventoryTransferStatusColors[transfer.status]}`}>
                            {InventoryTransferStatusLabels[transfer.status]}
                        </span>
                    </div>

                    {/* Vách ngăn nếu có nút tiếp theo */}
                    {(transfer.status === InventoryTransferStatus.Draft || transfer.status === InventoryTransferStatus.InTransit) && (
                ...
                {(transfer.status === InventoryTransferStatus.Draft || transfer.status === InventoryTransferStatus.InTransit) && (
                    ...
                    >
                        ...
                        {transfer.status === InventoryTransferStatus.InTransit ? 'Hủy & Hoàn Về Kho Nguồn' : 'Hủy Lệnh'}
                    </button>
                )}
            </div>
            ...
            {activeTab === 'info' && (
                <DetailCard>
                    <div className="p-8 flex flex-col gap-8 animate-in fade-in duration-300">
                        {transfer.status === InventoryTransferStatus.Cancelled && transfer.cancellationReason && (
                            <div className="bg-rose-50 border border-rose-200 p-4 rounded-xl flex gap-3 items-start shadow-sm">
                                <AlertCircle className="w-5 h-5 shrink-0 mt-0.5 text-rose-500" />
                                <div className="flex flex-col gap-1">
                                    <h4 className="font-bold text-sm text-rose-800 uppercase tracking-wider">Lý do hủy lệnh:</h4>
                                    <p className="text-sm text-rose-700 font-medium">{transfer.cancellationReason}</p>
                                </div>
                            </div>
                        )}
                        ...
                        <DetailSection title="Hành Trình Chuyển Kho" dotColor="bg-blue-400">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-6 gap-x-8">
                                <InfoField label="Kho nguồn (Xuất phát)" value={<span className="font-bold text-amber-700">{transfer.fromWarehouseName}</span>} />
                                <InfoField label="Kho đích (Tiếp nhận)" value={<span className="font-bold text-emerald-700">{transfer.toWarehouseName}</span>} />
                                <InfoField
                        ...
                        <DetailSection title="Tiến Độ Vận Chuyển" dotColor="bg-indigo-400">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-6 gap-x-8">
                                ...
                                <div className="flex flex-col items-start">
                                    <span className="text-[11px] font-bold text-slate-400 uppercase mb-1">Thời gian xuất phát</span>
                                    {transfer.dispatchedDate ? <DateTimeCell isoString={transfer.dispatchedDate} /> : <span className="font-medium text-slate-800">---</span>}
                                </div>
                                
                                <InfoField label="Thủ kho nhận hàng" value={transfer.receivedByName || '---'} />
                                <div className="flex flex-col items-start">
                                    <span className="text-[11px] font-bold text-slate-400 uppercase mb-1">Thời gian tiếp nhận</span>
                                    {transfer.receivedDate ? <DateTimeCell isoString={transfer.receivedDate} /> : <span className="font-medium text-slate-800">---</span>}
                                </div>
                            </div>

                            {transfer.note && (
                                <div className="mt-6 pt-6 border-t border-slate-100">
                                    <InfoField label="Ghi chú điều phối" value={<span className="italic text-slate-600 font-medium">{transfer.note}</span>} />
                                </div>
                            )}
                        </DetailSection>
                    </div>
                                    ...
                                    {transfer.details?.map((item, idx) => (
                                        <tr key={item.id || idx} className="hover:bg-slate-50/80 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-center border-l border-indigo-100 bg-indigo-50/20">
                                                <span className="font-black text-[15px] text-indigo-700">{item.quantity}</span>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6">
                            <p className="text-sm text-slate-600 mb-5 leading-relaxed">
                                Bạn đang thực hiện hủy lệnh chuyển kho <strong className="text-slate-900 bg-slate-100 px-1.5 py-0.5 rounded">{transfer.transferCode}</strong>. 
                                Nếu hàng đang đi đường (InTransit), hệ thống sẽ tự động hoàn trả số lượng lại cho kho nguồn.
                            </p>
                            <div>
