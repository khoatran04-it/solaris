...
// Atomic Components
import { 
    ListPageContainer, ListHeader, ListCard, TableLoading, 
    TableEmpty, ListPagination, DateTimeCell 
...
const UoMConversionList: React.FC = () => {
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [debouncedSearch, standardFilter, productFilter, statusFilter, createdAtFilter, updatedAtFilter]);
    ...
    }, [currentPage, debouncedSearch, standardFilter, productFilter, statusFilter, createdAtFilter, updatedAtFilter]);
    ...
    return (    
        <ListPageContainer>
            ...
            <ListCard>
                        ...
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                <th className="w-[15%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <CustomFilter title="PHÂN LOẠI" options={standardOptions} selectedValues={standardFilter} onApply={setStandardFilter} />
                                </th>
                                
                                <th className="w-[20%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Công thức quy đổi</th>
                                
                                <th className="w-[20%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <CustomFilter title="ÁP DỤNG CHO" options={productOptions} selectedValues={productFilter} onApply={setProductFilter} />
                                </th>
                                        ...
                                        <td className="py-4 px-4">
                                            {item.productId ? (
                                                <div className="flex flex-col">
                                                    <span className="font-bold text-slate-700 text-sm line-clamp-1">{item.productName}</span>
                                                    <span className="text-[11px] font-semibold text-slate-400">{item.productCode}</span>
                                                </div>
                                            ) : (
                                                <span className="text-sm font-semibold text-slate-400 italic">Mọi sản phẩm</span>
                                            )}
                                        </td>
                ...
                <ListPagination 
                    currentPage={currentPage} totalPages={totalPages} totalItems={data.length} 
                    onPageChange={setCurrentPage} isLoading={isLoading}
                />
            </ListCard>

            <ConfirmDeleteModal 
                isOpen={isModalOpen} itemName="dữ liệu quy đổi này" 
                onClose={() => setIsModalOpen(false)} onConfirm={confirmDelete} 
            />
        </ListPageContainer>
    );
};
