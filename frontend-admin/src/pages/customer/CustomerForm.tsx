...
const INITIAL_STATE: CustomerPayload = {
    ...
    addresses: [{
        receiverName: '', phone: '', province: '', district: '', ward: '', streetAddress: '', isDefault: true
    }]
};
...
const CustomerForm: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (isEditMode && id) {
            customerApi.getById(Number(id)).then(res => {
                setFormData({
                    code: res.code || '',
                    name: res.name || '',
    ...
    const validateForm = () => {
        ...
        else if (existingPhones.includes(trimmedPhone) && (!isEditMode || trimmedPhone !== originalPhone)) {
        ...
        if (!isEditMode && formData.addresses && formData.addresses.length > 0) {
            ...
            if (addr.receiverName || addr.phone || addr.province || addr.streetAddress) {
                if (!addr.receiverName.trim()) newErrors.address_receiverName = 'Vui lòng nhập tên người nhận.';
                ...
                if (!addr.streetAddress.trim()) newErrors.address_streetAddress = 'Vui lòng nhập số nhà, tên đường.';
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const cleanPayload: CustomerPayload = {
                ...
                addresses: (!isEditMode && formData.addresses![0].streetAddress.trim()) 
                           ? formData.addresses 
                           : undefined
            };
    ...
    return (
        <PageContainer>
            ...
            <FormHeader 
                ...
                subtitle={isEditMode ? 'Cập nhật thông tin và phân hạng' : 'Tạo mới hồ sơ khách hàng vào hệ thống'}
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-10">
                    ...
                    <FormSection title="Thông Tin Cá Nhân">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <FormInput 
                                label="Mã KH" required placeholder="KH-001"
                            ...
                            />
                            <FormInput 
                                label="Số điện thoại" required placeholder="0901234567"
                                value={formData.phoneNumber} error={errors.phoneNumber} disabled={loading}
                                onChange={e => handleFieldChange('phoneNumber', e.target.value)} 
                            />
                            <FormInput 
                                label="Email" placeholder="email@example.com"
                                value={formData.email || ''} error={errors.email} disabled={loading}
                            ...
                            <CustomDatePicker 
                                ...
                                onChange={(date) => handleFieldChange('birthday', date ? date.toLocaleDateString('en-CA') : null)}
                            />

                            <FormSelect 
                                label="Giới tính" placeholder="Chọn giới tính"
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 flex flex-col gap-6">
                                <div className="grid grid-cols-2 gap-6">
                                    <FormSelect 
                                        label="Loại khách hàng" required placeholder="Chọn loại..." showSearch
                                        options={typeOptions} value={formData.customerTypeId} error={errors.customerTypeId}
                                        onSelect={val => handleFieldChange('customerTypeId', val)} 
                                    />
                                    <FormSelect 
                                        label="Bậc xếp hạng" required placeholder="Chọn bậc hạng..." showSearch
                                        options={tierOptions} value={formData.customerTierId} error={errors.customerTierId}
                                ...
                                <div className="flex flex-col gap-3 p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                                    <label className="font-bold text-[13px] text-slate-700 uppercase tracking-wide">Gắn Nhóm Tiếp Thị (Tùy chọn)</label>
                                    <div className="flex flex-wrap gap-2.5">
                                        ...
                                        {groupOptions.length === 0 && <span className="text-sm text-slate-400 italic">Chưa có nhóm nào trong hệ thống.</span>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-6">
                                <FormSelect 
                                    label="Trạng thái tài khoản" required 
                                    value={formData.isActive ? 1 : 0} options={STATUS_OPTIONS}
                                    onSelect={val => handleFieldChange('isActive', val === 1)}
                                />
                                <FormInput 
                                    label="Mã số thuế" placeholder="VD: 5800..."
                                    value={formData.taxCode || ''} onChange={e => handleFieldChange('taxCode', e.target.value)} 
                                />
                            </div>
                        </div>
                    </FormSection>
                        ...
                        <FormSection title="Địa Chỉ Mặc Định (Tùy chọn)">
                            <div className="p-6 rounded-2xl border border-blue-100 bg-blue-50/30">
                                <div className="flex items-center gap-2 mb-4 text-blue-800">
                                    <MapPin size={18} />
                                    <span className="text-sm font-bold">Thiết lập địa chỉ giao hàng đầu tiên cho khách</span>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    <div className="lg:col-span-2">
                                        <FormInput 
                                            label="Người nhận" placeholder="Tên người nhận..."
                                            value={formData.addresses![0].receiverName} error={errors.address_receiverName}
                                            onChange={e => handleAddressChange('receiverName', e.target.value)} 
                                        />
                                    </div>
                                    <div className="lg:col-span-2">
                                        <FormInput 
                                            label="SĐT nhận hàng" placeholder="09..."
                                            value={formData.addresses![0].phone} error={errors.address_phone}
                                            onChange={e => handleAddressChange('phone', e.target.value)} 
                                        />
                                    </div>
                                    <FormInput 
                                        label="Tỉnh/Thành" placeholder="Hồ Chí Minh"
                                        value={formData.addresses![0].province} error={errors.address_province}
                                        onChange={e => handleAddressChange('province', e.target.value)} 
                                    />
                                    <FormInput 
                                        label="Quận/Huyện" placeholder="Quận 1"
                                        value={formData.addresses![0].district} error={errors.address_district}
                                        onChange={e => handleAddressChange('district', e.target.value)} 
                                    />
                                    <FormInput 
                                        label="Phường/Xã" placeholder="Phường Bến Nghé"
                                        value={formData.addresses![0].ward} error={errors.address_ward}
                                        onChange={e => handleAddressChange('ward', e.target.value)} 
                                    />
                                    <div className="lg:col-span-4">
                                        <FormInput 
                                            label="Địa chỉ chi tiết (Số nhà, đường)" placeholder="Số 12, Lê Duẩn..."
                                            value={formData.addresses![0].streetAddress} error={errors.address_streetAddress}
                    ...
                    <FormSection title="Ghi Chú & Khác">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <FormInput 
                                label="Link Ảnh Đại Diện (Avatar URL)" placeholder="https://..."
                                value={formData.avatarPath || ''} onChange={e => handleFieldChange('avatarPath', e.target.value)} 
                            />
                            <FormTextarea 
                                label="Ghi chú nội bộ" placeholder="Khách hàng khó tính, cần bọc hàng kỹ..." 
                                value={formData.note || ''} rows={1} 
