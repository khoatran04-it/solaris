...
import { Users, MapPin, History, Hexagon, Loader2, User, Plus, Star, Edit3, Trash2 } from 'lucide-react';
...
const CustomerDetail: React.FC = () => {
    ...
    return (
        <DetailPageContainer>
            ...
            <DetailHeader 
                icon={Hexagon} title="Hồ Sơ Khách Hàng"
                subtitle={<>Mã hệ thống: <span className="text-slate-800 font-bold">{customer.code}</span></>}
                onBack={() => navigate('/customers')}
            />

            <TabGroup>
                <TabButton active={activeTab === 'detail'} onClick={() => setActiveTab('detail')} label="THÔNG TIN HỒ SƠ" icon={Users} />
                <TabButton active={activeTab === 'addresses'} onClick={() => setActiveTab('addresses')} label="SỔ ĐỊA CHỈ GIAO HÀNG" icon={MapPin} />
                <TabButton active={activeTab === 'history'} onClick={() => setActiveTab('history')} label="LỊCH SỬ MUA HÀNG" icon={History} />
            </TabGroup>
            ...
            <DetailCard>
                ...
                {activeTab === 'detail' && (
                    <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
                        <div className="lg:col-span-2 flex flex-col gap-9">
                            <DetailSection title="Tổng Quan Cá Nhân" dotColor="bg-yellow-400">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-8">
                                    ...
                                    <InfoField label="Giới tính" value={customer.gender === true ? 'Nam' : customer.gender === false ? 'Nữ' : 'Chưa cập nhật'} />
                                    <InfoField label="Ngày sinh" value={customer.birthday ? new Date(customer.birthday).toLocaleDateString('vi-VN') : '---'} />
                                    ...
                                    <div className="md:col-span-2 grid grid-cols-2 border-l-2 border-slate-100 pl-4 py-0.5 gap-2 mt-2">
                                        <div className="flex flex-col items-start"><span className="text-[11px] font-bold text-slate-400 uppercase mb-1">Ngày Khởi Tạo</span><DateTimeCell isoString={customer.createdAt} /></div>
                                        <div className="flex flex-col items-start"><span className="text-[11px] font-bold text-slate-400 uppercase mb-1">Cập Nhật Gần Nhất</span><DateTimeCell isoString={customer.updatedAt} /></div>
                                    </div>
                                </div>
                            </DetailSection>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <DetailSection title="Định Danh Hệ Thống" dotColor="bg-purple-400">
                                    <div className="space-y-4">
                                        <InfoField label="Loại Khách Hàng" value={<span className="inline-flex px-2.5 py-0.5 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-bold border border-indigo-200/30">{customer.customerTypeName}</span>} />
                                        <InfoField label="Bậc Xếp Hạng" value={<span className="inline-flex px-2.5 py-0.5 rounded-lg bg-amber-50 text-amber-700 text-xs font-bold border border-amber-200/30">{customer.customerTierName}</span>} />
                                        <div className="flex flex-col gap-1.5 pt-2">
                                            <span className="text-[12px] font-bold text-slate-400 uppercase">Nhóm Marketing</span>
                                            <div className="flex flex-wrap gap-1.5">
                                                {customer.groups && customer.groups.length > 0 ? (
                                                    customer.groups.map((group, idx) => (
                                                        <span key={idx} className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-semibold bg-blue-50 text-blue-600 border border-blue-100">{group}</span>
                                                    ))
                                ...
                                <DetailSection title="Ghi Chú Nội Bộ" dotColor="bg-slate-400">
                                    <p className="text-sm text-slate-600 leading-relaxed bg-slate-50/70 p-4 rounded-2xl border border-slate-100 min-h-35">
                                        {customer.note || <span className="italic text-slate-400 font-normal">Không có ghi chú đặc biệt cho khách hàng này.</span>}
                                    </p>
                                </DetailSection>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-1">
                            <DetailProfileCard 
                                title={customer.name} subTitle={customer.code} logoPath={customer.avatarPath} fallbackIcon={User} statusActive={customer.isActive}
                            />
                        </div>
                    </div>
                )}
                    ...
                    <div className="p-8 flex flex-col gap-6">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 mt-1">Quản lý các điểm nhận hàng của khách hàng</p>
                            </div>
                            <button 
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50/70 border-b border-slate-100">
                                        ...
                                        <th className="w-[20%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Người Nhận</th>
                                        <th className="w-[15%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">SĐT</th>
                                        <th className="w-[40%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Địa Chỉ Chi Tiết</th>
                ...
                {activeTab === 'history' && (
                    <TabEmptyPlaceholder icon={History} title="Lịch sử mua hàng trống" desc="Khách hàng này chưa phát sinh bất kỳ đơn hàng hay giao dịch nào trên hệ thống." />
                )}
            </DetailCard>
        </DetailPageContainer>
    );
