...
const InventoryIssueForm: React.FC = () => {
  ...
  const handleAutoSuggestBatch = async (rowId: string, variantId: number | '') => {
    if (!formData.warehouseId) return showToast('warning', 'Vui lòng chọn Kho xuất ở mục (1) trước!');
                    ...
                    return (
                      <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-2 text-right font-black text-slate-800">
                            {((Number(row.quantity || 0) * Number(row.unitPrice || 0))).toLocaleString('vi-VN')} đ
                        </td>
