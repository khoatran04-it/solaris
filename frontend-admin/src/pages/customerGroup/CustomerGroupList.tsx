...
const CustomerGroupList: React.FC = () => {
  ...
  const [deletingRecord, setDeletingRecord] = useState<CustomerGroup | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await customerGroupApi.getAll({
  ...
  const confirmDelete = async () => {
    ...
    try {
      await customerGroupApi.delete(deletingRecord.id);
  ...
  return (
    <ListPageContainer>
      ...
      <ListHeader
        ...
        subtitle="Quản lý hồ sơ nhóm, theo dõi trạng thái hoạt động của nhóm"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-100">
                ...
                <th className="w-[22%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">
                  Tên Nhóm KH
                </th>
                <th className="w-[22%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">
                  Mô tả
              ...
              {isLoading ? (
                ...
                data.map((item) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 text-slate-500 text-sm truncate max-w-50">
                      {item.description || '---'}
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex justify-center gap-1.5 opacity-40 group-hover:opacity-100 transition-all duration-300">
                        <button
                          onClick={() => navigate(`/customer-groups/edit/${item.id}`)}
                ...
                <TableEmpty colSpan={7} message="Không tìm thấy nhóm khách hàng phù hợp." />
              )}
            </tbody>
          </table>
        </div>
