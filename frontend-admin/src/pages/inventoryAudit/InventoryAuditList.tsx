                    ...
                    <table className="w-full text-left border-collapse min-w-325">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                <th className="w-[12%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã Kiểm Kê</th>
                                ...
                                <th className="w-[12%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <CustomFilter title="HÌNH THỨC" options={typeOptions} selectedValues={typeFilter} onApply={setTypeFilter} />
                                </th>
                                ...
                                <th className="w-[8%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Lệch SL</th>
                                
                                <th className="w-[10%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Giá Trị Lệch</th>
                                
                                <th className="w-[8%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider hidden lg:table-cell text-center">Người Lập</th>
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm">
                            {loading ? (
                                ...
                                <TableEmpty colSpan={10} message="Thử thay đổi từ khóa tìm kiếm hoặc điều kiện lọc." />
                            ) : (
                                audits.map((audit) => {
                                    ...
                                    return (
                                        <tr key={audit.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                            ...
                                            <td className="py-3 px-2">
                                                <div className="text-[13px] font-bold text-slate-800">{audit.warehouseName}</div>
                                            </td>
                                            ...
                                            <td className="py-3 px-2 text-center">
                                                <div className="flex justify-center">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold border shadow-sm ${InventoryAuditStatusColors[audit.status]}`}>
                                            ...
                                            <td className="py-3 px-2 text-right">
                                                <span className={`text-[14px] font-black ${isNegative ? 'text-rose-600' : isPositive ? 'text-emerald-600' : 'text-slate-500'}`}>
                                            ...
                                            <td className="py-3 px-2 text-right">
                                                <span className={`text-[14px] font-black ${isNegative ? 'text-rose-600' : isPositive ? 'text-emerald-600' : 'text-slate-500'}`}>
                                                    {audit.totalVarianceAmount > 0 ? '+' : ''}{formatCurrency(audit.totalVarianceAmount)}
                                                </span>
                                            </td>
                                            
                                            {/* CELL 9: NGƯỜI LẬP */}
                                            <td className="py-3 px-2 text-center hidden lg:table-cell">
                                                <div className="text-[12px] text-slate-600 font-medium truncate max-w-30" title={audit.auditorName}>
                                                    {audit.auditorName}
                                                </div>
                                            </td>
