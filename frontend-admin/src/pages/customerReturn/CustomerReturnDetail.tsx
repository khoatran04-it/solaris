...
const CustomerReturnDetail: React.FC = () => {
    ...
    const fetchReturn = useCallback(async () => {
        ...
        try {
            ...
            if (data.details) {
                setInspectionItems(data.details.map(d => ({
                    detailId: d.id,
    ...
    const handleInspectionChange = (detailId: number, field: keyof CustomerReturnItemInspectionPayload, val: any) => {
    ...
    if (!ret) return <div className="p-12 text-center text-rose-500 font-bold">Không tìm thấy chứng từ trả hàng!</div>;
    ...
    return (
        <DetailPageContainer>
            ...
            <DetailHeader
                ...
                subtitle={<>Mã chứng từ: <span className="font-bold text-slate-800">{ret.returnCode}</span></>}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6 p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border shadow-sm ${CustomerReturnStatusColors[ret.status]}`}>
                    ...
                    {/* Vách ngăn */}
                    {(ret.status === CustomerReturnStatus.Pending || ret.status === CustomerReturnStatus.Inspecting) && (
                    ...
                    <div className="p-8 flex flex-col gap-8 animate-in fade-in duration-300">
                        <DetailSection title="Thông Tin Tiếp Nhận Hàng" dotColor="bg-blue-400">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-6 gap-x-8">
                                ...
                                />
                                <InfoField label="Khách hàng" value={<span className="font-bold text-slate-800">{ret.customerName}</span>} />
                                <InfoField label="Kho tiếp nhận" value={<span className="font-bold text-indigo-700">{ret.warehouseName}</span>} />
                        ...
                        <DetailSection title="Lý Do & Đánh Giá Chất Lượng" dotColor="bg-emerald-400">
                            <div className="flex flex-col gap-4">
                                <InfoField label="Lý do khách trả hàng" value={<span className="text-slate-800 font-medium">{ret.reason || '---'}</span>} />
                                {ret.inspectionNotes && (
                                    <div className="p-5 bg-slate-50 border border-slate-200 rounded-2xl">
                                        <div className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Ghi chú kiểm định QC:</div>
                                        <div className="text-sm font-medium text-slate-700 leading-relaxed">{ret.inspectionNotes}</div>
                                    </div>
                                )}
                            </div>
                        </DetailSection>
                            ...
                            <table className="w-full text-left whitespace-nowrap">
                                <thead className="bg-slate-50 text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="px-4 py-4 text-center bg-emerald-50/50 text-emerald-800 min-w-35">SL Đạt (Nhập Tốt)</th>
                                        <th className="px-4 py-4 text-center bg-amber-50/50 text-amber-800 min-w-35">SL Hỏng (Vào Kho Lỗi)</th>
                                ...
                                <tbody className="divide-y divide-slate-100 text-sm">
                                    {ret.details?.map((item, idx) => (
                                        <tr key={item.id || idx} className="hover:bg-slate-50/80 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-center font-bold text-slate-700">{item.returnedQuantity}</td>
                                            ...
                                            <td className="px-4 py-3 text-right font-black text-rose-600">{formatCurrency(item.refundAmount)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot className="bg-slate-50/80 border-t border-slate-200 text-sm">
                                    <tr className="border-t border-slate-200 bg-rose-50/30">
                                        <td colSpan={9} className="px-4 py-4 text-right text-rose-800 uppercase tracking-wider text-xs font-black">Tổng Tiền Hoàn Khách Hàng:</td>
                                        <td className="px-4 py-4 text-right text-[17px] font-black text-rose-600">{formatCurrency(ret.refundAmount)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                ...
                <div className="fixed inset-0 z-60 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 overflow-y-auto animate-in fade-in">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden my-8 animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-4 border-b border-indigo-100 bg-indigo-50/50 flex items-center justify-between">
                            ...
                            <span className="text-xs font-bold text-slate-500 bg-white px-2.5 py-1 rounded-lg border border-slate-200">Mã: {ret.returnCode}</span>
                        </div>
                        <div className="p-6 max-h-[70vh] overflow-y-auto">
                            <p className="text-xs text-slate-600 mb-5 leading-relaxed bg-indigo-50/30 p-3 rounded-xl border border-indigo-100">
                                💡 <strong>Hướng dẫn:</strong> Phân loại số lượng hàng đạt tiêu chuẩn (sẽ cộng lại vào <strong>Tồn kho Khả Dụng</strong>) và số lượng hàng hư hỏng (sẽ cộng vào <strong>Tồn kho Hàng Lỗi/Hỏng</strong>).
                            </p>
                                ...
                                <table className="w-full text-left text-xs whitespace-nowrap">
                                    <thead className="bg-slate-50 text-slate-600 font-bold uppercase tracking-wider border-b border-slate-200">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 text-center text-emerald-700 bg-emerald-50/50">SL Đạt (Tốt)</th>
                                            <th className="px-4 py-3 text-center text-amber-700 bg-amber-50/50">SL Hỏng (Lỗi)</th>
                                            ...
                                            return (
                                                <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                                                    <td className="px-4 py-3">
                                                        ...
                                                        <div className="text-[11px] font-semibold text-indigo-600">Lô: {d.batchCode || '---'}</div>
                                                    </td>
                                                    ...
                                                    <td className="px-4 py-3 text-center bg-emerald-50/20 border-l border-emerald-100">
                                                        <input
                                                            ...
                                                            onChange={(e) => handleInspectionChange(d.id, 'acceptedQuantity', parseFloat(e.target.value) || 0)}
                                                    ...
                                                    <td className="px-4 py-3 text-center bg-amber-50/20 border-l border-amber-100">
                                                        <input
                                                            ...
                                                            onChange={(e) => handleInspectionChange(d.id, 'damagedQuantity', parseFloat(e.target.value) || 0)}
                                                    ...
                                                    <td className="px-4 py-3 border-l border-slate-100">
                                                        <input
                                                            ...
                                                            onChange={(e) => handleInspectionChange(d.id, 'rejectReason', e.target.value)}
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Kết luận & Biên bản kiểm định chung</label>
                                <textarea
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6">
                            <p className="text-sm text-slate-600 mb-5 leading-relaxed">
                                Bạn đang từ chối tiếp nhận trả hàng cho phiếu <strong className="text-slate-900 bg-slate-100 px-1.5 py-0.5 rounded">{ret.returnCode}</strong>.
                            </p>
                            <div>
