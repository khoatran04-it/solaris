...
const SupplierTypeForm: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (isEditMode && id) {
            supplierTypeApi.getById(Number(id)).then(res => {
                    setFormData({
                        code: res.code || '',
                        name: res.name || '',
    ...
    return (
        <PageContainer>
            ...
            <FormHeader 
                ...
                subtitle={isEditMode ? 'Cập nhật thông tin danh mục nhà cung cấp' : 'Thiết lập danh mục nhà cung cấp mới cho hệ thống'}
