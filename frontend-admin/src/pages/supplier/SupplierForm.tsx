...
const INITIAL_STATE: SupplierPayload = {
    ...
    addresses: [{
        contactName: '', contactPhone: '', province: '', district: '', ward: '', streetAddress: '', isDefault: true
    }]
};
    ...
    const validateForm = () => {
        ...
        if (!isEditMode && formData.addresses && formData.addresses.length > 0) {
            ...
            if (addr.contactName || addr.contactPhone || addr.province || addr.streetAddress) {
                if (!addr.contactName.trim()) newErrors.address_contactName = 'Vui lòng nhập người liên hệ.';
                if (!addr.contactPhone.trim()) newErrors.address_contactPhone = 'Vui lòng nhập SĐT liên hệ kho.';
    ...
    return (
        <PageContainer>
            ...
            <FormHeader 
                ...
                subtitle={isEditMode ? 'Cập nhật thông tin đối tác' : 'Thêm mới đối tác cung ứng vào hệ thống'}
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-10">
                    ...
                    <FormSection title="Thông Tin Doanh Nghiệp">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <FormInput 
                                label="Mã NCC" required placeholder="NCC-001"
                                value={formData.code} error={errors.code} disabled={loading || isEditMode}
                                onChange={e => handleFieldChange('code', e.target.value)} 
                            />
                            <FormInput 
                                label="Tên Công Ty/Đối tác" required placeholder="Công ty TNHH..."
                                value={formData.name} error={errors.name} disabled={loading}
                                onChange={e => handleFieldChange('name', e.target.value)} 
                            />
                            <FormSelect 
                                label="Phân Loại" required placeholder="Chọn loại..." showSearch
                                options={typeOptions} value={formData.supplierTypeId} error={errors.supplierTypeId}
                                onSelect={val => handleFieldChange('supplierTypeId', val)} 
                            />
                            <FormInput 
                                label="Số điện thoại chính" required placeholder="0901234567"
                                value={formData.phone} error={errors.phone} disabled={loading}
                                onChange={e => handleFieldChange('phone', e.target.value)} 
                            />
                            <FormInput 
                                label="Email công ty" required placeholder="contact@company.com"
                                value={formData.email || ''} error={errors.email} disabled={loading}
                                onChange={e => handleFieldChange('email', e.target.value)} 
                            />
                            <FormInput 
                                label="Mã số thuế" placeholder="VD: 0312..."
                            ...
                            />
                            <FormSelect 
                                label="Trạng thái hợp tác" required 
                                value={formData.isActive ? 1 : 0} options={STATUS_OPTIONS}
                                onSelect={val => handleFieldChange('isActive', val === 1)}
                            />
                            <FormInput 
                                label="Logo" placeholder="https://..."
                                value={formData.logoPath || ''} onChange={e => handleFieldChange('logoPath', e.target.value)} 
                            />
                        </div>
                    </FormSection>
                            ...
                            <div className="p-6 rounded-2xl border border-blue-100 bg-blue-50/30">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    <div className="lg:col-span-2">
                                        <FormInput 
                                            label="Người phụ trách kho" placeholder="Tên thủ kho..."
                                            value={formData.addresses![0].contactName} error={errors.address_contactName}
                                            onChange={e => handleAddressChange('contactName', e.target.value)} 
                                        />
                                    </div>
                                    <div className="lg:col-span-2">
                                        <FormInput 
                                            label="SĐT phụ trách" placeholder="09..."
                                            value={formData.addresses![0].contactPhone} error={errors.address_contactPhone}
                    ...
                    <FormSection title="Thanh Toán & Ghi Chú">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <FormInput 
                                label="Số tài khoản (VNĐ)" placeholder="1903..."
                                value={formData.bankAccount || ''} onChange={e => handleFieldChange('bankAccount', e.target.value)} 
                            />
                            <FormInput 
                                label="Ngân hàng & Chi nhánh" placeholder="Techcombank Chi nhánh..."
                                value={formData.bankName || ''} onChange={e => handleFieldChange('bankName', e.target.value)} 
                            />
                            <div className="md:col-span-2">
                                <FormTextarea 
                                    label="Ghi chú nội bộ" placeholder="Chính sách công nợ, đánh giá NCC..." 
                                    value={formData.note || ''} rows={2} 
