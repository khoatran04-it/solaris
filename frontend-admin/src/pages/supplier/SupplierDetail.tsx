...
import { Building2, MapPin, Hexagon, Loader2, Plus, Star, Edit3, Trash2, Truck, Package, Clock } from 'lucide-react';
...
const SupplierDetail: React.FC = () => {
    ...
    // --- STATES CHO MODAL SẢN PHẨM / BẢNG GIÁ ---
    const [isProductModalOpen, setIsProductModalOpen] = useState(false);
    const [editingProduct, setEditingProduct] = useState<SupplierProduct | null>(null);

    // --- STATES CHO MODAL XÓA ---
    const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
    const [deletingType, setDeletingType] = useState<'address' | 'product'>('address');
    const [deletingAddressId, setDeletingAddressId] = useState<number | null>(null);
    const [deletingProductRecord, setDeletingProductRecord] = useState<SupplierProduct | null>(null);
    ...
    const fetchSupplierProducts = () => {
        ...
        supplierProductApi.getBySupplierId(Number(id), false)
            .then(res => setSupplierProducts(res))
            .catch(() => showToast('error', 'Không thể tải bảng giá sản phẩm của NCC!'))
    ...
    // --- HANDLERS: SẢN PHẨM & BẢNG GIÁ ---
    const openAddProductModal = () => {
        setEditingProduct(null);
        setIsProductModalOpen(true);
    };

    const openEditProductModal = (prod: SupplierProduct) => {
        setEditingProduct(prod);
        setIsProductModalOpen(true);
    };

    const handleSaveProduct = async (payload: SupplierProductPayload) => {
        try {
            if (editingProduct) {
                await supplierProductApi.update(editingProduct.id, payload);
                showToast('success', 'Cập nhật giá sản phẩm thành công!');
            } else {
            ...
            showToast('error', error?.response?.data?.message || 'Có lỗi xảy ra khi lưu bảng giá.');
            throw error;
        }
    };

    const handleToggleProductActive = async (prodId: number, currentStatus: boolean) => {
        try {
            await supplierProductApi.toggleActive(prodId);
            fetchSupplierProducts();
    ...
    const confirmDelete = async () => {
        if (deletingType === 'address' && deletingAddressId) {
            try {
        ...
        } else if (deletingType === 'product' && deletingProductRecord) {
            try {
                await supplierProductApi.delete(deletingProductRecord.id);
                setIsDeleteModalOpen(false);
                showToast('success', 'Đã xóa sản phẩm khỏi danh mục của NCC!');
                fetchSupplierProducts();
            } catch (error: any) {
                showToast('error', error?.response?.data?.message || 'Không thể xóa cấu hình giá này.');
            }
        }
    };
    ...
    return (
        <DetailPageContainer>
            ...
            <ConfirmDeleteModal 
                ...
                itemName={deletingType === 'address' ? "địa chỉ kho này" : (deletingProductRecord?.variantName || "sản phẩm này")} 
            ...
            <TabGroup>
                ...
                <TabButton active={activeTab === 'products'} onClick={() => setActiveTab('products')} label={`SẢN PHẨM & BẢNG GIÁ (${supplierProducts.length})`} icon={Package} />
                <TabButton active={activeTab === 'addresses'} onClick={() => setActiveTab('addresses')} label={`DANH SÁCH ĐỊA CHỈ KHO (${supplier.addresses?.length || 0})`} icon={MapPin} />
            ...
            <DetailCard>
                ...
                {/* ================= TAB 2: SẢN PHẨM & BẢNG GIÁ ================= */}
                {activeTab === 'products' && (
                    <div className="p-8 flex flex-col gap-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-lg font-bold text-slate-800">Sản Phẩm & Bảng Giá Cung Ứng</h3>
                                <p className="text-sm text-slate-500 mt-1">Danh mục các mặt hàng được phép nhập từ nhà cung cấp này kèm giá tham chiếu và MOQ</p>
                            </div>
                            <button 
                                onClick={openAddProductModal}
                                className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold text-slate-900 bg-yellow-400 rounded-xl hover:bg-yellow-500 shadow-sm shadow-yellow-200 transition-all cursor-pointer"
                            >
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50/70 border-b border-slate-100">
                                        <th className="w-[30%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Sản Phẩm Cung Ứng</th>
                                        <th className="w-[15%] py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã SKU NCC</th>
                                        <th className="w-[15%] py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Đơn Giá Nhập</th>
                                        <th className="w-[12%] py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">ĐVT Mua</th>
                                        <th className="w-[12%] py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">MOQ & Giao</th>
                                        <th className="w-[10%] py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Trạng Thái</th>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {productsLoading ? (
                                        <tr>
                                            <td colSpan={7} className="py-8 text-center text-slate-400">Đang tải danh mục sản phẩm...</td>
                                        </tr>
                                    ) : supplierProducts && supplierProducts.length > 0 ? (
                                        supplierProducts.map((sp) => (
                                            <tr key={sp.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                                ...
                                                <td className="py-4 px-4 text-right">
                                                    <span className="text-sm font-extrabold text-slate-900">
                                                        {formatCurrency(sp.lastImportPrice)}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="py-4 px-4 text-center">
                                                    <div className="flex flex-col items-center gap-0.5">
                                                        <span className="text-[11px] font-bold text-slate-700">
                                                            Min: {sp.minimumOrderQuantity}
                                                        </span>
                                                        <span className="text-[10px] text-slate-400 flex items-center gap-1">
                                                            <Clock size={10} /> {sp.leadTimeDays} ngày
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="py-4 px-4 text-center">
                                                    ...
                                                    >
                                                        <span className={`w-1.5 h-1.5 rounded-full ${sp.isActive ? 'bg-emerald-500' : 'bg-slate-400'}`}></span>
                                                ...
                                                <td className="py-4 px-6 text-center">
                                                    <div className="flex justify-center gap-1.5 opacity-40 group-hover:opacity-100 transition-all duration-300">
                                                        ...
                                                        <button 
                                                            onClick={() => { setDeletingType('product'); setDeletingProductRecord(sp); setIsDeleteModalOpen(true); }}
                                                            className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
                                                            title="Xóa khỏi danh mục NCC"
                                                        >
                                        ...
                                        <TableEmpty colSpan={7} message="Nhà cung cấp này chưa có sản phẩm nào trong bảng giá." />
                                    )}
                                </tbody>
                            </table>
                        </div>
                                            ...
                                            <tr key={addr.id} className={`transition-colors duration-200 group ${addr.isDefault ? 'bg-yellow-50/30' : 'hover:bg-slate-50/80'}`}>
                                                ...
                                                <td className="py-4 px-6 text-center">
                                                    <button 
                                                        ...
                                                        className={`p-2 rounded-full transition-all ${
                                                            ...
                                                            : 'text-slate-300 hover:text-yellow-500 hover:bg-yellow-50 cursor-pointer'
                                                        }`}
                                                ...
                                                <td className="py-4 px-6">
                                                    <div className="flex justify-center gap-1.5 opacity-40 group-hover:opacity-100 transition-all duration-300">
                                                        ...
                                                        <button 
                                                            onClick={() => { setDeletingType('address'); setDeletingAddressId(addr.id); setIsDeleteModalOpen(true); }}
                                                            className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
                                                            title="Xóa"
                                                        >
                                                            <Trash2 size={17} strokeWidth={2.5} />
