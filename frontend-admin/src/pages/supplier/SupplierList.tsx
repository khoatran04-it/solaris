                    ...
                    <table className="w-full text-left border-collapse min-w-300">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100 text-center">
                                <th className="w-[10%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã ID</th>
                                <th className="w-[18%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Nhà Cung Cấp</th>
                                
                                <th className="w-[15%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">
                                    <CustomFilter 
                                        title="PHÂN LOẠI" options={typeOptions} 
                                        selectedValues={typeFilter} onApply={setTypeFilter} 
                                    />
                                </th>
                                
                                <th className="w-[10%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">SĐT</th>
                                
                                <th className="w-[12%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <div className="flex justify-center">
                                        <CustomFilter 
                                            title="TRẠNG THÁI" options={statusOptions} 
                                            selectedValues={statusFilter} onApply={setStatusFilter} 
                                        />
                                    </div>
                                </th>
                                ...
                                <th className="w-[11%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Thao Tác</th>
                            </tr>
                        </thead>
                            ...
                            {isLoading ? (
                                ...
                                data.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors duration-200 group">
                                        ...
                                        <td className="py-4 px-6 text-center text-slate-600 font-medium text-sm">{item.phone}</td>
                                ...
                                <TableEmpty colSpan={8} message="Thử thay đổi từ khóa tìm kiếm hoặc điều kiện lọc đối tác." />
                            )}
                        </tbody>
                    </table>
                </div>
