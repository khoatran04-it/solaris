...
const PromotionCampaignList: React.FC = () => {
    ...
    const getCampaignTimeStatus = (startStr: string, endStr: string, isActive: boolean) => {
        if (!isActive) return { text: 'Đã khóa', style: 'bg-slate-100 text-slate-500 border-slate-200' };

        const now = new Date().getTime();
        const start = new Date(startStr).getTime();
        const end = new Date(endStr).getTime();

        if (now < start) return { text: 'Sắp diễn ra', style: 'bg-amber-50 text-amber-600 border-amber-200/60' };
        if (now > end) return { text: 'Đã kết thúc', style: 'bg-slate-50 text-slate-400 border-slate-200 border-dashed' };
        
        return { text: 'Đang diễn ra', style: 'bg-emerald-50 text-emerald-700 border-emerald-200 animate-pulse-slow' };
    };
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/70 border-b border-slate-100">
                                <th className="w-[30%] py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Thông Tin Chiến Dịch</th>
                                ...
                                <th className="w-[15%] py-4 px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    <div className="flex justify-center">
                                        <CustomFilter title="TRẠNG THÁI (HỆ THỐNG)" options={statusOptions} selectedValues={statusFilter} onApply={setStatusFilter} />
                                    </div>
                                </th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {isLoading ? (
                                ...
                                data.map((item) => {
                                    const timeStatus = getCampaignTimeStatus(item.startDate, item.endDate, item.isActive);
