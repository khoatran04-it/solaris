...
import { Megaphone, Save, Plus, CheckSquare, Square, Search, ChevronRight, Box, Info, Package } from 'lucide-react';
...
const PromotionCampaignForm: React.FC = () => {
    ...
    const isAllFilteredSelected = filteredVariants.length > 0 && filteredVariants.every(v => formData.variantIds?.includes(v.id));
                        ...
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                            <FormSection title="Thiết Lập Chiến Dịch">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <FormInput label="Tên chiến dịch" required value={formData.name} onChange={e => handleFieldChange('name', e.target.value)} />
                                    <FormSelect 
                                        label="Loại khuyến mãi" options={DISCOUNT_TYPE_OPTIONS} 
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <CustomDatePicker label="Kết thúc vào" value={new Date(formData.endDate)} onChange={d => handleFieldChange('endDate', d?.toISOString())} />
                                    </div>
                                </div>
                                <div className="mt-6">
                                    <FormTextarea label="Mô tả chiến dịch" value={formData.description || ''} onChange={(e:any) => handleFieldChange('description', e.target.value)} />
                                </div>
                            </FormSection>
                        </div>
                    )}
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50 p-4 border-b border-slate-200">
                                        ...
                                        >
                                            ...
                                            <span className="text-sm font-bold text-slate-700">
                                                {isAllFilteredSelected ? 'Bỏ chọn tất cả' : 'Chọn tất cả'} <span className="font-normal text-slate-500">({filteredVariants.length})</span>
                                            </span>
                                        </button>
                                    ...
                                    <div className="max-h-112.5 overflow-y-auto divide-y divide-slate-100 bg-slate-50/30">
                                        {filteredVariants.length === 0 ? (
                                            <div className="p-12 text-center flex flex-col items-center">
                                                ...
                                                <p className="text-slate-400 text-sm font-medium">Không tìm thấy sản phẩm nào phù hợp với từ khóa.</p>
                                            </div>
                                        ) : (
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="shrink-0 ml-2">
                                                            {isSelected ? <CheckSquare className="text-yellow-500" size={20}/> : <Square className="text-slate-300" size={20}/>}
                                                        </div>
                                                        <div className="w-11 h-11 rounded-lg border border-slate-200 flex items-center justify-center overflow-hidden bg-white shrink-0 shadow-sm">
                                                            {v.imagePath ? <img src={v.imagePath} alt="" className="w-full h-full object-cover"/> : <Box size={20} className="text-slate-300" />}
                                                        </div>
                                                        <div className="flex-1 min-w-0 flex flex-col">
                                                            <h4 className="text-[14px] font-bold text-slate-800 truncate leading-snug">{v.name}</h4>
                                                            <div className="flex items-center gap-2 mt-1">
                                                                <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">{v.code}</span>
                                                            </div>
                                                        </div>
                                                        <div className="text-right pr-4 shrink-0 flex flex-col items-end">
                                                            ...
                                                            {defaultPriceInfo ? (
                                                                <span className="text-[14px] font-black text-slate-700">
                                                                    ...
                                                                    <span className="text-[11px] font-medium text-slate-400 ml-1">/ {defaultPriceInfo.uoMName}</span>
                                                                </span>
                                                            ) : (
