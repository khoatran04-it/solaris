...
const InventoryReceiptDetail: React.FC = () => {
    ...
    const fetchReceipt = () => {
        setLoading(true);
        inventoryReceiptApi.getById(Number(id))
    ...
    if (!receipt) return <div className="p-12 text-center text-rose-500 font-bold">Không tìm thấy chứng từ!</div>;
    ...
    return (
        <DetailPageContainer>
            ...
            {/* ================= TABS ĐIỀU HƯỚNG ================= */}
            <TabGroup>
                <TabButton active={activeTab === 'info'} onClick={() => setActiveTab('info')} label="1. THÔNG TIN PHIẾU" icon={FileText} />
                <TabButton active={activeTab === 'details'} onClick={() => setActiveTab('details')} label={`2. CHI TIẾT KIỂM ĐẾM (${receipt.details?.length || 0})`} icon={Package} />
            </TabGroup>
                    ...
                    <div className="p-8 flex flex-col gap-8 animate-in fade-in duration-300">
                        {receipt.status === InventoryReceiptStatus.Cancelled && receipt.cancellationReason && (
                            <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 flex gap-3 items-start shadow-sm">
                                ...
                                <div className="flex flex-col gap-1">
                                    <span className="text-sm font-bold text-rose-800 uppercase tracking-wider">Lý do hủy phiếu:</span>
                                    <span className="text-sm font-medium text-rose-700">{receipt.cancellationReason}</span>
                                </div>
                            </div>
                        )}
                        ...
                        <DetailSection title="Thông Tin Chung" dotColor="bg-indigo-400">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-6 gap-x-8">
                                <InfoField label="Mã phiếu nhập" value={<span className="font-bold text-indigo-700">{receipt.receiptCode}</span>} />
                                <InfoField label="Kho nhận hàng" value={<span className="font-bold text-slate-800">{receipt.warehouseName}</span>} />
                        ...
                        <DetailSection title="Ghi Chú Kiểm Đếm" dotColor="bg-slate-400">
                            <div className="text-sm text-slate-600 leading-relaxed bg-slate-50/70 p-5 rounded-2xl border border-slate-100 min-h-24">
                                {receipt.note || <span className="italic text-slate-400 font-medium">Không có ghi chú nào được ghi nhận.</span>}
                            </div>
                        </DetailSection>
                    </div>
                )}
                            ...
                            <table className="w-full text-left whitespace-nowrap min-w-[900px]">
                                <thead>
                                    <tr className="bg-slate-50/80 border-b border-slate-200">
                                        ...
                                        <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã SKU</th>
                                        <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Tên SP</th>
                                        <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-left">Mã Lô</th>
                                        <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">ĐVT</th>
                                        <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right bg-slate-100/50">SL Dự Kiến</th>
                                        <th className="py-4 px-4 text-xs font-bold text-emerald-600 uppercase tracking-wider text-right bg-emerald-50/50">SL Đạt ✅</th>
                                        <th className="py-4 px-4 text-xs font-bold text-rose-600 uppercase tracking-wider text-right bg-rose-50/50">Từ Chối ❌</th>
                                        <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-left min-w-50">Lý Do Từ Chối</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                    {receipt.details && receipt.details.length > 0 ? (
                                        receipt.details.map((detail, index) => (
                                            <tr key={detail.id || index} className="hover:bg-slate-50 transition-colors group">
                                                <td className="py-4 px-4 text-sm text-slate-400 text-center font-medium">{index + 1}</td>
                                                <td className="py-4 px-4 text-sm font-bold text-slate-700">{detail.variantCode || '---'}</td>
                                                <td className="py-4 px-4 text-sm font-medium text-slate-900">{detail.variantName}</td>
                                                ...
                                                <td className="py-4 px-4 text-sm text-slate-600 truncate max-w-62.5 border-l border-slate-100" title={detail.rejectReason}>
                                                    {detail.rejectReason || <span className="italic text-slate-400">---</span>}
                                                </td>
                                            </tr>
                                        ))
                                    ) : (
                                ...
                                {receipt.details && receipt.details.length > 0 && (
                                    <tfoot className="bg-slate-50 border-t border-slate-200">
                                        <tr>
                                            <td colSpan={5} className="py-4 px-4 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Tổng Số Lượng:</td>
                                            <td className="py-4 px-4 text-right text-[15px] font-black text-slate-700">{totalExpected.toLocaleString('vi-VN')}</td>
                                            <td className="py-4 px-4 text-right text-[16px] font-black text-emerald-600">{totalAccepted.toLocaleString('vi-VN')}</td>
                                            <td className="py-4 px-4 text-right text-[16px] font-black text-rose-600">{totalRejected.toLocaleString('vi-VN')}</td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
                                )}
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6">
                            <p className="text-sm text-slate-600 mb-5 leading-relaxed">
                                Hành động này sẽ đánh dấu phiếu nhập kho <strong className="text-slate-900 bg-slate-100 px-1 rounded">{receipt.receiptCode}</strong> thành Đã Hủy. Hệ thống yêu cầu ghi rõ lý do.
                            </p>
                            <div>
