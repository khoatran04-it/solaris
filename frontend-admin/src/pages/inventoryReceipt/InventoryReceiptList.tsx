...
const InventoryReceiptList: React.FC = () => {
  ...
  // --- EFFECTS ---
  useEffect(() => {
    Promise.all([warehouseApi.getAllList(), supplierApi.getAllList()])
              ...
              {isLoading ? (
                ...
                data.map((receipt) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-2">
                      <div
                        className="text-[13px] text-slate-600 truncate max-w-[180px]"
                    ...
                    <td className="py-3 px-2 text-center">
                      <div className="flex justify-center">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold border shadow-sm ${InventoryReceiptStatusColors[receipt.status]}`}
                        >
                ...
                <TableEmpty colSpan={8} message="Không tìm thấy phiếu nhập kho nào phù hợp." />
              )}
            </tbody>
          </table>
        </div>
