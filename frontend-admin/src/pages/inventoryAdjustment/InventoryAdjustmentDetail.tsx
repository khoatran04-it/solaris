...
const InventoryAdjustmentDetail: React.FC = () => {
    ...
    if (loading) return <div className="p-12 text-center text-slate-500 font-medium">Đang tải dữ liệu phiếu điều chỉnh...</div>;
    if (!adj) return <div className="p-12 text-center text-rose-500 font-bold">Không tìm thấy chứng từ điều chỉnh!</div>;

    const totalAmount = adj.details?.reduce((sum, item) => sum + (Number(item.totalAmount) || 0), 0) || 0;
                            ...
                            <table className="w-full text-left whitespace-nowrap">
                                ...
                                <tbody className="divide-y divide-slate-100 text-sm">
                                    {adj.details?.map((item, idx) => {
                                        const isIncrease = item.adjustmentType === InventoryAdjustmentType.IncreaseAvailable;
                                        return (
                                            <tr key={item.id || idx} className="hover:bg-slate-50">
                                                ...
                                                <td className="px-4 py-3">
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${
                                                        isIncrease ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
                                                    }`}>
                                                ...
                                                <td className="px-4 py-3 text-right font-bold text-slate-900">{formatCurrency(item.totalAmount)}</td>
                                                <td className="px-4 py-3 text-xs italic text-slate-600">{item.reasonDetail || '---'}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                                <tfoot className="bg-slate-50/80 border-t border-slate-200 font-bold text-sm">
                                    <tr>
                                        <td colSpan={8} className="px-4 py-4 text-right text-slate-800 uppercase tracking-wider text-xs font-bold">Tổng Giá Trị Điều Chỉnh:</td>
                                        <td colSpan={2} className="px-4 py-4 text-left text-lg font-black text-slate-900">{formatCurrency(totalAmount)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                        ...
                        <DetailSection title="Thông Tin Quản Trị">
                            ...
                            {adj.note && (
                                <div className="mt-6 pt-6 border-t border-slate-100">
                                    <InfoField label="Ghi chú & Biên bản giải trình" value={<span className="italic text-slate-600">{adj.note}</span>} />
                                </div>
                            )}
                        </DetailSection>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
                        ...
                        <div className="p-6">
                            <p className="text-sm text-slate-600 mb-4">
                                Bạn đang thực hiện hủy phiếu điều chỉnh <strong className="text-slate-900">{adj.adjustmentCode}</strong>.
                            </p>
