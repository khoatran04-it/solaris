    ...
    return (
        <PageContainer>
            ...
            <FormHeader 
                ...
                subtitle={isEditMode ? 'Cập nhật thông tin gốc của hàng hóa trước khi cấu hình biến thể' : 'Tạo mới một hồ sơ sản phẩm (Parent Product) vào hệ thống'}
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-10">
                    ...
                    <FormSection title="Định Danh & Phân Loại">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <FormInput 
                                label="Mã Sản Phẩm (SKU)" required placeholder="VD: IPHONE-15"
                                value={formData.code} error={errors.code} disabled={loading}
                                onChange={e => handleFieldChange('code', e.target.value)} 
                            />
                            <FormInput 
                                label="Tên Sản Phẩm" required placeholder="VD: Apple iPhone 15 Pro Max"
                            ...
                            <FormSelect 
                                ...
                                placeholder="Chọn danh mục..." showSearch
                                value={formData.categoryId || 0} options={categoryOptions}
                                onSelect={val => handleFieldChange('categoryId', val)}
                            />
                            <FormSelect 
                                label="Đơn vị tính cơ bản" required
                                placeholder="VD: Cái, Hộp, Chiếc..." showSearch
                                value={formData.baseUoMId} options={uomOptions} error={errors.baseUoMId}
                    ...
                    <FormSection title="Hình Ảnh & Mô Tả">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-6">
                                <FormInput 
                                    label="Link Ảnh Sản Phẩm (URL)" placeholder="https://..."
                                ...
                                {formData.imagePath && (
                                    <div className="p-3 border border-slate-200 rounded-xl bg-slate-50 w-max">
                                        <img 
                                            ...
                                            onError={(e: any) => e.target.src = 'https://placehold.co/128x128?text=Lỗi+Ảnh'}
                                        />
                                    </div>
                                )}
                            </div>
                            ...
                            <FormTextarea 
                                label="Mô tả sản phẩm" placeholder="Nhập chi tiết về sản phẩm này..." 
