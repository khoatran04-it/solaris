...
import { Lock, User, Phone, Mail, ArrowRight, AlertCircle, Sparkles } from 'lucide-react';
    ...
    return (
        <div className="max-w-md w-full bg-white rounded-3xl shadow-[0_10px_35px_-5px_rgba(0,0,0,0.06)] border border-slate-200/80 p-8 sm:p-9 space-y-6">
            ...
            <form onSubmit={handleRegister} className="space-y-3.5">
                ...
                <div className="space-y-1.5">
                    <label className="font-extrabold text-xs text-slate-700 uppercase tracking-wide block">Email (Tùy chọn)</label>
                    <div className="relative">
                        <input
                            type="email"
                ...
                <div className="space-y-1.5">
                    <label className="font-extrabold text-xs text-slate-700 uppercase tracking-wide block">Mật khẩu (tối thiểu 6 ký tự) *</label>
                    <div className="relative">
                        <input
                            type="password"
                            placeholder="••••••••"
                ...
                <div className="space-y-1.5">
                    <label className="font-extrabold text-xs text-slate-700 uppercase tracking-wide block">Xác nhận mật khẩu *</label>
                    <div className="relative">
                        <input
                            type="password"
                            placeholder="••••••••"
            ...
            <div className="text-center pt-3 border-t border-slate-100 text-xs text-slate-600">
                <p>
                    Đã có tài khoản?{' '}
                    <Link href={`/dang-nhap?redirect=${encodeURIComponent(redirectUrl)}`} className="text-emerald-700 hover:text-emerald-800 font-bold underline">
                        Đăng nhập ngay
                    </Link>
                </p>
            </div>
...
export default function DangKyPage() {
    return (
        <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 bg-slate-50/50">
            <Suspense fallback={<div className="text-xs text-slate-500">Đang tải trang đăng ký...</div>}>
                <DangKyContent />
            </Suspense>
        </div>
    );
