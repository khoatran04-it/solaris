...
import { User, Package, MapPin, RotateCcw, CheckCircle2 } from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                    ...
                    <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-200 p-5 space-y-2 shadow-xs">
                        <Link
                            href="/tai-khoan"
                            className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-emerald-50 text-emerald-700 font-bold text-xs"
                        >
                        ...
                        <Link
                            ...
                            className="flex items-center gap-3 px-4 py-3 rounded-2xl text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium text-xs transition-colors"
                        >
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        ...
                        <div className="bg-gradient-to-r from-emerald-800 via-teal-800 to-slate-950 text-white rounded-3xl p-6 sm:p-8 shadow-lg space-y-6">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                                <div className="flex items-center gap-4">
                                    <div className="w-16 h-16 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center text-3xl shadow-inner border border-white/20 shrink-0">
                                        🎖️
                                    </div>
                                    <div>
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs">
                                    <span className="text-emerald-100 font-medium">
                                        {profile?.nextTierName ? (
                                            <>
                                                Mua thêm <strong className="text-amber-300 font-black">{(profile.amountToNextTier || 0).toLocaleString('vi-VN')} ₫</strong> để thăng cấp lên bậc <strong className="text-white font-extrabold">{profile.nextTierName}</strong>
                                            </>
                                        ) : (
                                            <span className="text-amber-300 font-bold">🌟 Bạn đã đạt thứ hạng thành viên VIP cao nhất!</span>
                                        )}
                                    </span>
                        ...
                        {/* Profile Form */}
                        <div className="bg-white rounded-3xl shadow-[0_2px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100 p-6 sm:p-8 space-y-6">
                            ...
                            <form onSubmit={handleSave} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="font-bold text-[13px] text-slate-700 uppercase tracking-wide mb-1 block">Họ và tên *</label>
                                    <input
                                        type="text"
                                        value={name}
                                ...
                                <div className="space-y-1">
                                    <label className="font-bold text-[13px] text-slate-700 uppercase tracking-wide mb-1 block">Địa chỉ Email</label>
                                    <input
                                        type="email"
                                        value={email}
                                ...
                                <div className="space-y-1">
                                    <label className="font-bold text-[13px] text-slate-700 uppercase tracking-wide mb-1 block">Giới tính</label>
                                    <select
                                        value={gender}
                                        onChange={(e) => setGender(e.target.value)}
                                        className="w-full h-[46px] px-4 rounded-xl border border-slate-200 text-sm bg-slate-50/50 hover:bg-white focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 focus:outline-none transition-all text-slate-800 font-medium"
                                    >
                                ...
                                <div className="sm:col-span-2 pt-4">
                                    <button
                                        type="submit"
                                        disabled={isSaving}
                                        className="px-6 h-[46px] bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm rounded-xl transition-all shadow-md active:scale-98 disabled:opacity-50"
                                    >
