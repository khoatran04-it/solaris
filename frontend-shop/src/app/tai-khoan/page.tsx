...
export default function TaiKhoanPage() {
  ...
  useEffect(() => {
    ...
    shopCustomerApi
      ...
      .then((prof: any) => {
        ...
        setEmail(prof.email || "");
        setGender(
          prof.gender === true ? "Nam" : prof.gender === false ? "Nữ" : "",
        );
      })
      .catch(() => {});
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const genderValue: boolean | null | undefined =
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <h3 className="text-xl font-black mt-1 tracking-tight">
                      {profile?.customerTierName ||
                        user.customerTierName ||
                        "Thành Viên Mới"}
                    </h3>
              ...
              <div className="pt-4 border-t border-white/10 space-y-2 relative z-10">
                    ...
                    {profile?.nextTierName ? (
                      <>
                        ...
                        <strong className="text-amber-300 font-black">
                          {(profile.amountToNextTier || 0).toLocaleString(
                            "vi-VN",
                          )}{" "}
                          ₫
                        </strong>{" "}
                ...
                <div className="w-full h-2.5 bg-white/20 rounded-full overflow-hidden p-0.5 backdrop-blur-xs">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.max(0, profile?.tierProgressPercent || 0))}%`,
                    }}
                  />
                </div>
              </div>
