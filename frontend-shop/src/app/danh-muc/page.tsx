...
export const metadata: Metadata = {
  ...
  description:
    "Khám phá hệ thống danh mục nông sản sạch, trái cây tươi ngon, rau củ quả hữu cơ đạt chuẩn VietGAP, GlobalGAP tại Solaris Farm.",
};
...
export default async function DanhMucOverviewPage() {
  ...
  const totalProducts = categories.reduce(
    (sum, g) =>
      sum + g.categories.reduce((cSum, c) => cSum + c.productCount, 0),
    0,
  );
      ...
      <div className="bg-gradient-to-br from-emerald-800 via-emerald-700 to-teal-800 rounded-3xl p-6 sm:p-10 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl space-y-3">
          ...
          <p className="text-xs sm:text-sm text-emerald-100 leading-relaxed font-normal">
            Khám phá hơn {totalProducts} mặt hàng nông sản tươi ngon, truy xuất
            nguồn gốc minh bạch từ các nông trại công nghệ cao Đà Lạt và các hợp
            tác xã nông nghiệp sạch.
          </p>

          <div className="flex items-center gap-4 pt-1 text-xs font-semibold text-emerald-200">
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-amber-300" /> 100% kiểm định
              QC
            </span>
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-amber-300" /> Giao lạnh 2H
            </span>
          </div>
        </div>
      </div>
        ...
        {categories.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {categories.map((group) => {
              const groupProductCount = group.categories.reduce(
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Bao gồm {group.categories.length} phân loại sản phẩm
                        chuyên biệt
                      </p>
                    </div>
