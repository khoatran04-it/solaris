...
function KetQuaContent() {
    ...
    return (
      <div className="max-w-xl mx-auto py-20 px-4 text-center space-y-4">
        ...
        <p className="text-xs font-bold text-slate-600">
          Đang đối soát giao dịch với VNPay Gateway...
        </p>
      </div>
    );
  }
  ...
  const isSuccess =
    result?.isSuccess ?? searchParams.get("vnp_ResponseCode") === "00";
  const orderCode = result?.orderCode || searchParams.get("vnp_TxnRef");
  const amount =
    result?.amount || (Number(searchParams.get("vnp_Amount")) || 0) / 100;
  const transactionNo =
    result?.transactionNo || searchParams.get("vnp_TransactionNo");
