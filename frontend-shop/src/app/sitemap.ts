...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const productRoutes: MetadataRoute.Sitemap = (prodRes.items || []).map(
      (p: ShopProductCard) => ({
    ...
    const categoryRoutes: MetadataRoute.Sitemap = categories.flatMap(
      (g: ShopCategoryTree) => [
        {
          url: `${baseUrl}/danh-muc/${g.groupSlug}`,
          lastModified: now,
          changeFrequency: "weekly",
          priority: 0.7,
        },
    ...
    const promoRoutes: MetadataRoute.Sitemap = promos.map(
      (pr: ShopPromotionBadge) => ({
