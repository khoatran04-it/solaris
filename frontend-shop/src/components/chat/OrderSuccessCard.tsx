        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 space-y-3 text-xs">
            ...
            <div className="p-3 bg-white rounded-xl border border-emerald-100 space-y-1.5">
                <div className="flex justify-between">
                    ...
                    <span className="font-mono font-bold text-slate-900">{payload.orderCode}</span>
                </div>
                <div className="flex justify-between">
                    <span className="text-slate-500">Tổng tiền:</span>
                    <span className="font-bold text-emerald-700">{formatVND(payload.totalAmount)}</span>
                </div>
                {payload.paymentMethodName && (
                    <div className="flex justify-between">
                        <span className="text-slate-500">Hình thức:</span>
                        <span className="font-medium text-slate-700">{payload.paymentMethodName}</span>
                    </div>
                )}
            </div>
