...
export default function ChatMessageContent({ content, isUser = false }: ChatMessageContentProps) {
    ...
    const flushList = (key: string) => {
        ...
        if (currentList.type === 'ul') {
            elements.push(
                <ul key={key} className="space-y-1.5 my-1.5 pl-4 list-disc marker:text-emerald-600">
            ...
            elements.push(
                <ol key={key} className="space-y-1.5 my-1.5 pl-4 list-decimal marker:text-emerald-600 marker:font-bold">
                    {currentList.items.map((item, idx) => (
                        <li key={idx} className="leading-relaxed">
                            {parseInlineFormatting(item)}
                        </li>
    ...
    lines.forEach((line, index) => {
        ...
        if (trimmed.startsWith('### ')) {
            elements.push(
                <h4 key={`h4-${index}`} className="font-extrabold text-slate-900 text-xs mt-2 mb-1 tracking-tight">
        ...
        if (trimmed.startsWith('## ')) {
            elements.push(
                <h3 key={`h3-${index}`} className="font-extrabold text-slate-900 text-sm mt-2.5 mb-1 tracking-tight">
        ...
        if (trimmed.startsWith('# ')) {
            elements.push(
                <h2 key={`h2-${index}`} className="font-black text-slate-900 text-base mt-3 mb-1 tracking-tight">
...
function parseInlineFormatting(text: string): React.ReactNode[] {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        // Inline code: `code`
            ...
            const inner = tokenStr.substring(1, tokenStr.length - 1);
            tokens.push(
                <code key={match.index} className="px-1.5 py-0.5 bg-slate-100 text-emerald-800 rounded font-mono text-[11px]">
                    {inner}
                </code>
            );
        }
        // Link: [label](url)
        else if (tokenStr.startsWith('[') && tokenStr.includes('](') && tokenStr.endsWith(')')) {
