...
describe('Module 13 - shopOrderApi Client', () => {
  ...
  it('TC04 - cancel gửi POST request tới /orders/:orderCode/cancel với lý do hủy', async () => {
    const cancelPayload = { reason: 'Khách hàng đổi ý muốn chọn sản phẩm khác' };
    ...
    expect(axiosClient.post).toHaveBeenCalledWith('/orders/ORD-20260830-010/cancel', cancelPayload);
    expect(result).toEqual(mockResponse);
  });
});
