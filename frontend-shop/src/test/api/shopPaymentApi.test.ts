...
describe('Module 14 - Shop Payment API Client (VNPay)', () => {
    ...
    it('TC01 - createVnPayUrl gửi POST /payment/vnpay/create-url và nhận paymentUrl', async () => {
        ...
        const mockResponse = {
            orderCode: 'ORD-20260830-001',
            paymentUrl: 'https://sandbox.vnpayment.vn/paymentv2/vpcpay.html?vnp_TmnCode=VVRIW1BA'
        };
        (axiosClient.post as any).mockResolvedValue(mockResponse);

        const res = await shopPaymentApi.createVnPayUrl(mockPayload);

        expect(axiosClient.post).toHaveBeenCalledWith('/payment/vnpay/create-url', mockPayload);
    ...
    it('TC02 - getVnPayCallback gửi GET /payment/vnpay/callback kèm query string', async () => {
        const queryStr = '?vnp_Amount=25000000&vnp_ResponseCode=00&vnp_TxnRef=ORD-20260830-001';
        const mockResponse = {
            isSuccess: true,
            orderCode: 'ORD-20260830-001',
            amount: 250000,
        ...
        expect(axiosClient.get).toHaveBeenCalledWith(`/payment/vnpay/callback${queryStr}`);
