...
describe('Module 13 - OrderDetailPage Component', () => {
  ...
  beforeEach(() => {
    ...
    (shopOrderApi.cancel as any).mockResolvedValue({ message: 'Hủy đơn hàng thành công' });
  });
  ...
  it('TC01 - Chưa đăng nhập: chuyển hướng sang /dang-nhap?redirect=/tai-khoan/don-hang/:orderCode', async () => {
    ...
    await waitFor(() => {
      expect(mockPush).toHaveBeenCalledWith('/dang-nhap?redirect=/tai-khoan/don-hang/ORD-20260830-100');
    });
  });
  ...
  it('TC03 - Hiển thị nút Hủy đơn hàng khi trạng thái là "Đã xác nhận"', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /Hủy đơn hàng/i })).toBeInTheDocument();
    });
  });
