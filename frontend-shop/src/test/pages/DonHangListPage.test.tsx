...
describe('Module 13 - DonHangListPage Component', () => {
  ...
  it('TC01 - Chưa đăng nhập: chuyển hướng sang /dang-nhap?redirect=/tai-khoan/don-hang', async () => {
    ...
    await waitFor(() => {
      expect(mockPush).toHaveBeenCalledWith('/dang-nhap?redirect=/tai-khoan/don-hang');
    });
  });
  ...
  it('TC02 - Đã đăng nhập: render danh sách đơn hàng với mã đơn, badge trạng thái và tổng tiền', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('2 x Bơ Booth 7 Đắk Lắk (Kg)')).toBeInTheDocument();
      expect(screen.getByText('3 x Xoài Cát Hòa Lộc (Hộp 1kg)')).toBeInTheDocument();
  ...
  it('TC04 - Mỗi đơn hàng có liên kết điều hướng xem chi tiết', async () => {
    ...
    await waitFor(() => {
      ...
      expect(viewLinks[0]).toHaveAttribute('href', '/tai-khoan/don-hang/ORD-20260830-001');
      expect(viewLinks[1]).toHaveAttribute('href', '/tai-khoan/don-hang/ORD-20260830-002');
    });
  });
});
