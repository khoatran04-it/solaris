...
describe('Module 15 - ProductCardMini & OrderSuccessCard (UI Testing in RAM)', () => {
    ...
    it('TC02 - Render OrderSuccessCard với mã đơn hàng và link chuyển hướng', () => {
        const mockPayload = {
            ...
            paymentUrl: 'https://sandbox.vnpayment.vn/paymentv2/vpcpay.html?vnp_Amount=35000000'
        };

        render(<OrderSuccessCard payload={mockPayload} />);

        expect(screen.getByText('Đặt hàng thành công qua AI Chatbot!')).toBeInTheDocument();
        ...
        expect(screen.getByRole('link', { name: /Cổng VNPay/i })).toHaveAttribute('href', mockPayload.paymentUrl);
        expect(screen.getByRole('link', { name: /Xem đơn hàng/i })).toHaveAttribute('href', '/tai-khoan/don-hang/ORD-SUCCESS-888');
    });
});
