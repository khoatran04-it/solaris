...
import {
  ShopCart,
  ShopCartAddPayload,
  ShopCartUpdatePayload,
  ShopSyncGuestCartPayload,
} from "@/types/cart";

const shopCartApi = {
  getCart: () => axiosClient.get<ShopCart>("/cart"),
  ...
  clearCart: () => axiosClient.delete<ShopCart>("/cart"),

  syncGuestCart: (data: ShopSyncGuestCartPayload) =>
    axiosClient.post<ShopCart>("/cart/sync", data),

  getGuestCartPreview: (data: ShopSyncGuestCartPayload) =>
    axiosClient.post<ShopCart>("/cart/guest-preview", data),
};
