...
const shopAiApi = {
  ...
  getSessionMessages: (sessionId: number, sessionToken?: string) =>
    axiosClient.get<ChatMessage[]>(`/ai/sessions/${sessionId}/messages`, {
      params: { sessionToken },
    }),
  ...
  deleteSession: (sessionId: number, sessionToken?: string) =>
    axiosClient.delete<{ success: boolean }>(`/ai/sessions/${sessionId}`, {
      params: { sessionToken },
    }),
  ...
  confirmOrder: (payload: ConfirmInteractiveOrderPayload) =>
    axiosClient.post<ConfirmInteractiveOrderResponse>(
      "/ai/confirm-order",
      payload,
    ),
};
