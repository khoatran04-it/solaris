...
namespace backend.Services
{
    ...
    public class InventoryTransferService : IInventoryTransferService
    {
        ...
        /// <inheritdoc />
        public async Task<bool> InspectAndReceiveTransferAsync(int id, int inspectedById, InventoryTransferInspectReceiveDto dto)
        {
            return await _context.ExecuteInTransactionAsync(async () =>
            {
                var transfer = await _context.InventoryTransfers
                ...
                if (transfer.Status != InventoryTransferStatus.InTransit)
                    throw new InvalidOperationException("Phiếu chuyển phải ở trạng thái Đang vận chuyển (InTransit) mới có thể kiểm đếm nhận hàng.");

                var userExists = await _context.IAUsers.AnyAsync(u => u.Id == inspectedById && !u.IsDeleted);
                if (!userExists)
                {
                    var firstUser = await _context.IAUsers.FirstOrDefaultAsync(u => !u.IsDeleted);
                    inspectedById = firstUser?.Id ?? 1;
                }

                var inspectDict = dto.Items?.ToDictionary(x => x.DetailId, x => x) ?? new();

                foreach (var detail in transfer.Details)
                {
                    decimal actualReceived = detail.Quantity;
                    decimal damaged = 0;

                    if (inspectDict.TryGetValue(detail.Id, out var inspectItem))
                    {
                        actualReceived = inspectItem.ActualReceivedQuantity;
                        damaged = inspectItem.DamagedQuantity;
                    }

                    detail.ActualReceivedQuantity = actualReceived;
                    detail.DamagedQuantity = damaged;

                    decimal actualBaseQty = await _uomConversionService.ConvertToBaseQuantityAsync(detail.VariantId, detail.UoMId, actualReceived);
                    decimal damagedBaseQty = await _uomConversionService.ConvertToBaseQuantityAsync(detail.VariantId, detail.UoMId, damaged);
                    ...
                    if (targetInv == null)
                    {
                        targetInv = new WarehouseInventory
                        {
                            ...
                            VariantId = detail.VariantId,
                            BatchId = detail.BatchId,
                            QuantityAvailable = actualBaseQty,
                            QuantityReserved = 0,
                            QuantityQC = 0,
                            QuantityDamaged = damagedBaseQty,
                            CreatedAt = DateTime.UtcNow,
                            UpdatedAt = DateTime.UtcNow
                        };
                        _context.WarehouseInventories.Add(targetInv);
                    ...
                    {
                        targetInv.QuantityAvailable += actualBaseQty;
                        targetInv.QuantityDamaged += damagedBaseQty;
                        targetInv.UpdatedAt = DateTime.UtcNow;
                    }
                    ...
                    if (actualBaseQty > 0)
                    {
                        _context.InventoryTransactions.Add(new InventoryTransaction
                        {
                            ...
                            ReferenceCode = transfer.TransferCode,
                            Note = $"Kiểm đếm nhận nguyên vẹn từ kho #{transfer.FromWarehouseId} (Phiếu {transfer.TransferCode}, {actualReceived} ĐVT -> {actualBaseQty} Base UoM)",
                ...
                transfer.Status = InventoryTransferStatus.Completed;
                transfer.ReceivedById = inspectedById;
                transfer.ReceivedDate = DateTime.UtcNow;
                transfer.InspectedById = inspectedById;
                transfer.InspectedDate = DateTime.UtcNow;
                if (!string.IsNullOrEmpty(dto.Note))
                {
                    transfer.Note = string.IsNullOrEmpty(transfer.Note) ? dto.Note : $"{transfer.Note} | Ghi chú kiểm đếm: {dto.Note}";
                }
                transfer.UpdatedAt = DateTime.UtcNow;
